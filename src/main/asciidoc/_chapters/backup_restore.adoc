...
link:https://issues.apache.org/jira/browse/HBASE-25707[HBASE-25707]
====
...
To improve usability, a retry mechanism has been introduced. If a backup operation fails to acquire the exclusive lock because another operation is in progress, it will automatically retry every second until a configurable timeout is reached.

The following configuration controls the timeout duration:
[source]
----
hbase.backup.exclusive.op.timeout.seconds (default: 3600)
----
If the lock is not acquired within the timeout, the operation will fail with an appropriate
error message. This reduces the need for users to implement custom retry logic.

NOTE: This feature was introduced in HBase 3.0.0-alpha
(see link:https://issues.apache.org/jira/browse/HBASE-19441[HBASE-19441
]) and is not available in the 2.x releases.
