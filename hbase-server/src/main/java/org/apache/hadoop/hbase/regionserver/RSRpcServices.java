...
public class RSRpcServices implements HBaseRPCErrorHandler, AdminService.BlockingInterface,
  ...
  private List<CellScannable> doNonAtomicRegionMutation(final HRegion region,
    ...
    for (ClientProtos.Action action : actions.getActionList()) {
      ...
      try {
        ...
        if (context != null && context.isRetryImmediatelySupported()
            && (context.getResponseCellSize() > maxQuotaResultSize || context.getResponseBlockSize()
                + context.getResponseExceptionSize() > maxQuotaResultSize)) {
  ...
  protected List<BlockingServiceAndInterface> getServices() {
    ...
    return new org.apache.hbase.thirdparty.com.google.common.collect.ImmutableList.Builder<BlockingServiceAndInterface>()
        .addAll(bssi).build();
  }
  ...
  public BulkLoadHFileResponse bulkLoadHFile(final RpcController controller,
    ...
    try {
      ...
      if (!request.hasBulkToken()) {
        ...
        // secure bulk load
        map = regionServer.getSecureBulkLoadManager().secureBulkLoadHFiles(region, request,
          clusterIds);
      }
          ...
          try {
            ...
            if (regionAction.getActionCount() == 1) {
              CheckAndMutateResult result =
                  checkAndMutate(region, quota, regionAction.getAction(0).getMutation(),
