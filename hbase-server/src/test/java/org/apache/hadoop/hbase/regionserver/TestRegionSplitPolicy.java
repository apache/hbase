...
public class TestRegionSplitPolicy {
  ...
  public void setupMocks() {
    ...
    doReturn(htd).when(mockRegion).getTableDescriptor();
  ...
  public void testForceSplitRegionWithReference() throws IOException {
    ...
    doReturn(true).when(mockRegion).shouldForceSplit();
    assertFalse(policy.shouldSplit());

    doReturn(false).when(mockRegion).shouldForceSplit();
    conf.set(HConstants.HBASE_REGION_SPLIT_POLICY_KEY,
  ...
  public void testCustomPolicy() throws IOException {
    ...
    HRegion myMockRegion = mock(HRegion.class);
    doReturn(myHtd).when(myMockRegion).getTableDescriptor();
    doReturn(stores).when(myMockRegion).getStores();
    ...
    doReturn(true).when(myMockRegion).shouldForceSplit();
    doReturn(Bytes.toBytes("efgh")).when(myMockRegion)
  ...
  public void testDelimitedKeyPrefixRegionSplitPolicy() throws IOException {
    ...
    doReturn(true).when(myMockRegion).shouldForceSplit();
    doReturn(Bytes.toBytes("efg,h")).when(myMockRegion)
        .getExplicitSplitPoint();
    ...
    doReturn(Bytes.toBytes("ijk")).when(myMockRegion)
    .getExplicitSplitPoint();
