...
class AsyncTableRegionLocatorImpl implements AsyncTableRegionLocator {
  ...
  public CompletableFuture<List<HRegionLocation>> getAllRegionLocations() {
    ...
    if (TableName.isMetaTableName(tableName)) {
      return AsyncCatalogTableAccessor.getTableHRegionLocations(
          conn.getTable(TableName.ROOT_TABLE_NAME), tableName);
    }
    return AsyncCatalogTableAccessor.getTableHRegionLocations(conn.getTable(TableName.META_TABLE_NAME),
      tableName);
  }
