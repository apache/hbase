 ...
 * 
 * <p>This test validates the complete multi-tenant HFile workflow:
 * <ol>
 ...
 * </ol>
 * 
 * <p>The test ensures tenant data isolation, format compliance, and data integrity 
 * across different access patterns (GET, SCAN, tenant-specific scans).
 */
...
public class MultiTenantHFileIntegrationTest {
  ...
  public void testMultiTenantHFileCreation() throws Exception {
    ...
    LOG.info("Test will process {} tenants with {} total expected rows", 
             TENANTS.length, calculateTotalExpectedRows());
   ...
   * Sets up table properties required for multi-tenant HFile functionality.
   */
   ...
   * Creates rows with format: {tenantId}row{paddedIndex} -> value_tenant-{tenantId}_row-{paddedIndex}
   */
  ...
  /**
   * Verify data integrity using individual GET operations.
   * Tests that each row can be retrieved correctly with expected values.
   */
  private void verifyDataWithGet() throws Exception {
    ...
    try (Connection conn = ConnectionFactory.createConnection(TEST_UTIL.getConfiguration());
      ...
      for (int tenantIndex = 0; tenantIndex < TENANTS.length; tenantIndex++) {
        ...
        LOG.info("Successfully verified {} GET operations for tenant {}", rowsForThisTenant, tenant);
      }
      
      LOG.info("GET verification completed: {}/{} rows verified successfully", 
               totalRowsVerified, calculateTotalExpectedRows());
    }
  }
  
  /**
   * Verify data integrity using full table SCAN.
   * Tests complete data retrieval and checks for tenant data mixing.
   */
  private void verifyDataWithScan() throws IOException {
    ...
    try (Connection connection = ConnectionFactory.createConnection(TEST_UTIL.getConfiguration());
      ...
      try (org.apache.hadoop.hbase.client.ResultScanner resultScanner = table.getScanner(tableScan)) {
        ...
        LOG.info("Full table SCAN completed: {}/{} rows scanned successfully", 
   ...
   * Tests that each tenant's data can be accessed independently without cross-tenant leakage.
   */
  private void verifyDataWithTenantSpecificScans() throws IOException {
    ...
    try (Connection connection = ConnectionFactory.createConnection(TEST_UTIL.getConfiguration());
      ...
      for (int tenantIndex = 0; tenantIndex < TENANTS.length; tenantIndex++) {
        ...
        LOG.info("Verifying tenant-specific scan for tenant {}: expecting {} rows", 
                 targetTenantId, expectedRowsForThisTenant);
        ...
        try (org.apache.hadoop.hbase.client.ResultScanner tenantScanner = table.getScanner(tenantScan)) {
          ...
          assertEquals("Row count mismatch for tenant " + targetTenantId, 
                      expectedRowsForThisTenant, tenantRowCount);
          
          LOG.info("Tenant {} scan successful: {}/{} rows verified", 
                   targetTenantId, tenantRowCount, expectedRowsForThisTenant);
        }
   ...
   * Tests non-existent tenant queries, empty scan behavior, and tenant boundary conditions.
   */
   ...
   * This test scans from the last row of one tenant to the first row of the next tenant
   * to ensure proper tenant isolation at boundaries.
   */
  private void verifyTenantBoundaries(Table table) throws IOException {
    ...
    for (int i = 0; i < TENANTS.length - 1; i++) {
      ...
      LOG.info("Testing boundary between tenant {} ({} rows) and tenant {} ({} rows)", 
               tenant1, tenant1RowCount, tenant2, tenant2RowCount);
      ...
      try (org.apache.hadoop.hbase.client.ResultScanner scanner = table.getScanner(scan)) {
        ...
        assertEquals("All scanned rows should belong to expected tenants", 
                    scannedRows.size(), totalFoundRows);
  ...
  /**
   * Verify HFile format and multi-tenant structure.
   * Validates that HFiles are properly formatted as v4 with tenant sections.
   */
  private void verifyHFileFormat(List<Path> hfilePaths) throws IOException {
    ...
    for (Path path : hfilePaths) {
      ...
      try (HFile.Reader reader = HFile.createReader(fs, path, cacheConf, true, conf)) {
        ...
        LOG.info("  Tenant sections with data: {}/{}", sectionsWithData, allTenantSectionIds.length);
    ...
    int expectedTotal = calculateTotalExpectedRows();
    assertEquals("Should have found all cells across all HFiles", expectedTotal, totalCellsFoundAcrossAllFiles);
  ...
  /**
   * Verify HFile metadata contains expected multi-tenant information.
   * Checks for section count, tenant index levels, and other v4 metadata.
   */
  private void verifyHFileMetadata(HFile.Reader reader, byte[][] allTenantSectionIds, 
    ...
    if (sectionCountBytes != null) {
      ...
      assertEquals("Section count should match found tenant sections", 
                  allTenantSectionIds.length, sectionCount);
    } else {
    ...
    assertEquals("Total section count should match found sections",
                allTenantSectionIds.length, mtReader.getTotalSectionCount());
   ...
   * Scans the filesystem to locate HFiles in the table's directory structure.
   */
          ...
          if (fs.exists(familyDir)) {
            ...
            for (FileStatus hfile : hfiles) {
              if (!hfile.getPath().getName().startsWith(".") && 
                ...
                LOG.info("Found HFile: {} (size: {} bytes)", 
