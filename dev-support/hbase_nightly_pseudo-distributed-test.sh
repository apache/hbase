...
set -e
function usage {
  echo "Usage: ${0} [options] /path/to/component/bin-install /path/to/hadoop/executable /path/to/hadoop/hadoop-yarn-server-tests-tests.jar /path/to/hadoop/hadoop-mapreduce-client-jobclient-tests.jar /path/to/hadoop/hadoop-yarn-server-timelineservice.jar"
  echo ""
...
yarn_server_timelineservice_jar="$(cd "$(dirname "$5")"; pwd)/$(basename "$5")"
...
if [ "${hadoop_version%.*.*}" -gt 2 ]; then
  if [ ! -f "${yarn_server_timelineservice_jar}" ]; then
    echo "Specified YARN server timeline service jar is not a file." >&2
    exit 1
  fi
  HADOOP_CLASSPATH="${yarn_server_timelineservice_jar}":"${yarn_server_tests_test_jar}" "${hadoop_exec}" jar "${mapred_jobclient_test_jar}" minicluster -format -writeConfig "${working_dir}/hbase-conf/core-site.xml" -writeDetails "${working_dir}/hadoop_cluster_info.json" >"${working_dir}/hadoop_cluster_command.out" 2>"${working_dir}/hadoop_cluster_command.err" &
else
