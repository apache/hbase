---
title: "Thrift API and Filter Language"
description: "Apache Thrift is a cross-platform, cross-language development framework. HBase includes a Thrift API and filter language. The Thrift API relies on client and server processes."
---

You can configure [Thrift](https://thrift.apache.org/) for secure authentication at the server and client side, by following the procedures in [Client-side Configuration for Secure Operation - Thrift Gateway](#security.client.thrift) and [Configure the Thrift Gateway to Authenticate on Behalf of the Client](#security.gateway.thrift).
