...
public class PerformanceEvaluation extends Configured implements Tool {
  ...
  static RunResult[] doLocalClients(final TestOptions opts, final Configuration conf)
    ...
    LOG.info(
      "Created " + opts.connCount + " connections for " + opts.numClientThreads + " threads");
  ...
  static abstract class TestBase {
    ...
    void testTakedown() throws IOException {
      ...
      synchronized (Test.class) {
        ...
        status.setStatus(
          "Latency (us) : " + YammerHistogramUtils.getHistogramReport(latencyHistogram));
        if (opts.replicas > 1) {
      ...
      try {
        if (opts.multiGet > 0) {
          ...
          if (this.gets.size() == opts.multiGet) {
            Result[] rs = this.table.get(this.gets).stream().map(f -> propagate(f::get))
                .toArray(Result[]::new);
            updateValueSize(rs);
            this.gets.clear();
          } else {
  ...
  static class MetaRandomReadTest extends MetaTest {
    ...
    boolean testRow(final int i, final long startTime) throws IOException, InterruptedException {
      ...
      HRegionLocation hRegionLocation =
          regionLocator.getRegionLocation(getSplitKey(rd.nextInt(opts.perClientRunRows)), true);
  ...
  static class MetaWriteTest extends MetaTest {
    ...
    boolean testRow(final int i, final long startTime) throws IOException {
      ...
      MetaTableAccessor.updateRegionLocation(connection, regionInfo,
        ServerName.valueOf("localhost", 60010, rand.nextLong()), i, System.currentTimeMillis());
      return true;
    }
  }
  ...
  private static String calculateMbps(int rows, long timeMs, final int valueSize, int families,
      int columns) {
    BigDecimal rowSize = BigDecimal.valueOf(
      ROW_LENGTH + ((valueSize + (FAMILY_NAME_BASE.length() + 1) + COLUMN_ZERO.length) * columns)
          * families);
    BigDecimal mbps =
        BigDecimal.valueOf(rows).multiply(rowSize, CXT).divide(BigDecimal.valueOf(timeMs), CXT)
            .multiply(MS_PER_SEC, CXT).divide(BYTES_PER_MB, CXT);
  ...
  protected static void printUsage(final String shortName, final String message) {
    ...
    System.err.println(
      " valueSize       Pass value size to use: Default: " + DEFAULT_OPTS.getValueSize());
    ...
    System.err.println(
      " bloomFilter     Bloom filter type, one of " + Arrays.toString(BloomType.values()));
