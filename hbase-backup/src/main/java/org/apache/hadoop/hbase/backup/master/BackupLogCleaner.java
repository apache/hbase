  ...
  private Map<Address, Long> getServersToOldestBackupMapping(List<BackupInfo> backups)
    ...
    for (BackupInfo backupInfo : backups) {
      for (TableName table : backupInfo.getTables()) {
        ...
        Set<Address> rsgroupServers = new HashSet<>(rsGroupInfo.getServers());
        if (rsGroupInfo != null && !rsgroupServers.isEmpty()) {
          servers.addAll(rsGroupInfo.getServers());
        } else {
          servers.addAll(conn.getAdmin()
            .getRegionServers().stream().map(s -> s.getAddress()).collect(Collectors.toList()));
        }
      }
    }
