...
public class Result implements CellScannable, CellScanner {
  ...
  public static void compareResults(Result res1, Result res2, boolean verbose) throws Exception {
    ...
    if (res1.size() != res2.size()) {
      if (verbose) {
        ...
        throw new Exception("This row doesn't have the same number of KVs: row="
            + Bytes.toStringBinary(res1.getRow()) + ", " + res1.size() + " cells are compared to "
            + res2.size() + " cells");
      }
    }
  ...
  @Override
  public Cell current() {
    if (isEmpty() || cellScannerIndex == INITIAL_CELLSCANNER_INDEX
