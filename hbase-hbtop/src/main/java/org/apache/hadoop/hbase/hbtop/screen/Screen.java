...
import org.apache.hadoop.hbase.hbtop.terminal.impl.LanternaTerminal;
...
/**
 * This dispatches key press events and time events to the current {@link ScreenView}.
 */
...
public class Screen implements Closeable {
  ...
  public Screen(Configuration conf, long initialRefreshDelay, Mode initialMode)
    ...
    this.terminal = new LanternaTerminal();
  ...
  public void run() {
    ...
    while (true) {
      try {
        KeyEvent keyEvent = terminal.pollKeyEvent();

        ScreenView nextScreenView;
        if (keyEvent != null) {
          // Dispatch the key event to the current screen
          nextScreenView = currentScreenView.handleKeyPress(keyEvent);
        } else {
          if (timerTimestamp != null) {
            long now = System.currentTimeMillis();
            if (timerTimestamp <= now) {
              // Dispatch the timer event to the current screen
              timerTimestamp = null;
