...
public class TestEncryptionKeyRotation {
  ...
  public void testCFKeyRotation() throws Exception {
    ...
    TableDescriptorBuilder tableDescriptorBuilder =
      TableDescriptorBuilder.newBuilder(TableName.valueOf("default", name));
    ...
    for (Path path : initialPaths) {
      assertTrue(Bytes.equals(initialCFKey.getEncoded(), extractHFileKey(path)),
    ...
    TEST_UTIL.waitFor(30000, new Waiter.Predicate<IOException>() {
      ...
      public boolean evaluate() throws IOException {
        return TEST_UTIL.getAdmin().getCompactionState(tableDescriptor.getTableName())
    ...
    for (Path path : pathsAfterCompaction) {
      assertTrue(Bytes.equals(secondCFKey.getEncoded(), extractHFileKey(path)),
        "Store file " + path + " has incorrect key");
    }
    ...
    for (Path path : compactedPaths) {
      assertTrue(Bytes.equals(initialCFKey.getEncoded(), extractHFileKey(path)),
        "Store file " + path + " retains initial key");
    }
  }
  ...
  private void createTableAndFlush(TableDescriptor tableDescriptor) throws Exception {
    ...
    try {
      table.put(new Put(Bytes.toBytes("testrow")).addColumn(cfd.getName(), Bytes.toBytes("q"),
        Bytes.toBytes("value")));
    } finally {
      table.close();
    }
