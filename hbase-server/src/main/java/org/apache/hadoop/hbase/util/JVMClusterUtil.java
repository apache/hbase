...
public class JVMClusterUtil {
  ...
  public static String startup(final List<JVMClusterUtil.MasterThread> masters,
    ...
    final int startTimeout =
        configuration != null
            ? Integer.parseInt(
              configuration.get("hbase.master.start.timeout.localHBaseCluster", "30000"))
            : 30000;
    ...
    final int initTimeout =
        configuration != null
            ? Integer.parseInt(
              configuration.get("hbase.master.init.timeout.localHBaseCluster", "200000"))
            : 200000;
        ...
        if (t.isAlive() && !wasInterrupted && now < maxTime) {
          try {
            ...
            LOG.info("Got InterruptedException on shutdown - "
                + "not waiting anymore on region server ends",
              e);
