...
public class HFileBlock implements Cacheable {
  ...
  HFileBlock unpack(HFileContext fileContext, FSReader reader) throws IOException {
    ...
    boolean succ = false;
    try {
      HFileBlockDecodingContext ctx =
          blockType == BlockType.ENCODED_DATA ? reader.getBlockDecodingContext()
  ...
  static class Writer implements ShipperListener {
    ...
    private void finishBlock() throws IOException {
      ...
      if (blockType == BlockType.DATA || blockType == BlockType.ENCODED_DATA) {
        compressAndEncryptDat = dataBlockEncodingCtx.compressAndEncrypt(baosInMemory.getBuffer(), 0,
          baosInMemory.size());
      } else {
    ...
    HFileBlock getBlockForCaching(CacheConfig cacheConf) {
      HFileContext newContext = new HFileContextBuilder().withBlockSize(fileContext.getBlocksize())
          .withBytesPerCheckSum(0).withChecksumType(ChecksumType.NULL) // no checksums in cached
                                                                       // data
  ...
  static class FSReaderImpl implements FSReader {
    ...
    public HFileBlock readBlockData(long offset, long onDiskSizeWithHeaderL, boolean pread,
      ...
      if (blk == null) {
        HFile.LOG
            .warn("HBase checksum verification failed for file " + pathName + " at offset " + offset
  ...
  public static int headerSize(boolean usesHBaseChecksum) {
    return usesHBaseChecksum ? HConstants.HFILEBLOCK_HEADER_SIZE
