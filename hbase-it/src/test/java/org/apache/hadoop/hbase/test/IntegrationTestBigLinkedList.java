      ...
      try (Connection conn = ConnectionFactory.createConnection(conf);
        ...
        if (!admin.tableExists(tableName)) {
          TableDescriptor tableDescriptor =
              TableDescriptorBuilder.newBuilder(getTableName(getConf()))
                  // if -DuseMob=true force all data through mob path.
                  .setColumnFamily(
                    setMobProperties(conf, ColumnFamilyDescriptorBuilder.newBuilder(FAMILY_NAME))
                        .build())
                  // Always add these families. Just skip writing to them when we do not test per CF
                  // flush.
                  .setColumnFamily(setMobProperties(conf,
                    ColumnFamilyDescriptorBuilder.newBuilder(BIG_FAMILY_NAME)).build())
                  .setColumnFamily(setMobProperties(conf,
                    ColumnFamilyDescriptorBuilder.newBuilder(TINY_FAMILY_NAME)).build())
                  .build();
  ...
  static class Verify extends Configured implements Tool {
    ...
    public static class VerifyMapper extends TableMapper<BytesWritable, BytesWritable> {
      ...
      protected void map(ImmutableBytesWritable key, Result value, Context context)
        ...
        if (multipleUnevenColumnFamilies && (!value.containsColumn(BIG_FAMILY_NAME, BIG_FAMILY_NAME)
