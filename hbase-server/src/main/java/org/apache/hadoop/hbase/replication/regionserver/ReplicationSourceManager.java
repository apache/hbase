          ...
          if (filtered != null && filtered.getEdit() != null && !filtered.getEdit().isEmpty()) {
            ...
            MetricsReplicationTableSource tableSource =
              source.getSingleSourceSourceByTable().computeIfAbsent(
                tableName.getMetricPrefixTableName(), t -> CompatibilitySingletonFactory
                  .getInstance(MetricsReplicationSourceFactory.class).getTableSource(t));
            // LOG.info("repl key=" + filtered.getKey() + ",edit=" +
            // edit.getCells().get(0).getClass() + ",entrySize=" + walReplAppendSize +
            // ",edit.heapsize=" + edit.heapSize() + ",keysize=" +key.estimatedSerializedSizeOf());
            tableSource.incrWalAppendBytes(walReplAppendSize);
            source.incrWalAppendBytes(walReplAppendSize);
          }
        }
      }
    } catch (Throwable t) {
