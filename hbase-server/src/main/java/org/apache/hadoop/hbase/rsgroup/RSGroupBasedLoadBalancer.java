...
public class RSGroupBasedLoadBalancer implements LoadBalancer {
  ...
  private ExecutorService groupBalancerPool;
   ...
   * not invoke {@link #tableBalancePlans(TableName, Map)}
   */
  @Override
  public List<RegionPlan> clusterBalancePlans(
    ...
    try {
      ...
      for (RSGroupInfo rsgroup : rsGroupInfoManager.listRSGroups()) {
        ...
        if (!loadOfTablesInGroup.isEmpty()) {
          ...
          groupPlans = this.internalBalancer.clusterBalancePlans(loadOfTablesInGroup);
        }
        if (groupPlans != null) {
  ...
  public void initialize() throws IOException {
    ...
    this.groupBalancerPool = Executors.newFixedThreadPool(
      config.getInt(CONF_RSGROUP_BALANCE_THREADS, 2),
  ...
  public void stop(String why) {
    ...
    if (this.groupBalancerPool != null) {
      this.groupBalancerPool.shutdownNow();
    }
    stopped = true;
  }
  ...
  public List<RegionPlan> tableBalancePlans(TableName tableName,
    ...
    List<RegionPlan> tablePlans =
        this.internalBalancer.tableBalancePlans(tableName, correctedLoadOfThisTable.get(tableName));
  ...
  private CompletableFuture<List<RegionPlan>> balanceRSGroupsParallel(List<RSGroupInfo> groups,
    boolean force) {
    return CompletableFuture.supplyAsync(() -> groups.stream()
      .map(groupInfo -> CompletableFuture.supplyAsync(() -> {
        try {
  ...
  public List<RegionPlan> balanceRSGroup(String groupName, boolean force) throws IOException {
    ...
    // try lock group balance
    ReadWriteLock lock = groupLocks.getLock(groupInfo.getName());
    if (lock.writeLock().tryLock()) {
      try {
        if (masterServices.getAssignmentManager().isMetaRegionInTransition()) {
          LOG.info("Not running balancer because meta regions in transition");
          return sucPlans;
        }
        ...
        List<RegionPlan> plans = clusterBalancePlans(assignmentsByTable);
        boolean balancerRan = !plans.isEmpty();
        if (balancerRan) {
          LOG.info("RSGroup balance {} starting with plan count: {}", groupName, plans.size());
          sucPlans = masterServices.executeRegionPlansWithThrottling(plans);
