...
public class KeyValue implements ExtendedCell, Cloneable {
  ...
  public static enum Type {
    ...
    Delete((byte) 8), DeleteFamilyVersion((byte) 10), DeleteColumn((byte) 12),
  ...
  public KeyValue(final byte[] row, final int roffset, final int rlength, final byte[] family,
      ...
      final int qlength, final long timestamp, final Type type, final byte[] value,
  ...
  public KeyValue(final byte[] row, final int roffset, final int rlength, final byte[] family,
      final int foffset, final int flength, final byte[] qualifier, final int qoffset,
      final int qlength, final long timestamp, final Type type, final byte[] value,
      final int voffset, final int vlength, final byte[] tags, final int tagsOffset,
  ...
  public KeyValue(Cell c) {
    this(c.getRowArray(), c.getRowOffset(), c.getRowLength(), c.getFamilyArray(),
        ...
        c.getQualifierLength(), c.getTimestamp(), Type.codeToType(c.getTypeByte()),
  ...
  private static byte[] createByteArray(final byte[] row, final int roffset, final int rlength,
      ...
      final int voffset, int vlength, byte[] tags, int tagsOffset, int tagsLength) {
  ...
  private static byte[] createByteArray(final byte[] row, final int roffset, final int rlength,
      ...
      final int qoffset, int qlength, final long timestamp, final Type type, final Object value,
      final int voffset, int vlength, List<Tag> tags) {
  ...
  public static class KVComparator implements RawComparator<Cell>, SamePrefixComparator<byte[]> {
    ...
    public int compareKey(Cell cell, byte[] row, int roff, int rlen, byte[] fam, int foff, int flen,
      ...
      int compare = compareRows(cell.getRowArray(), cell.getRowOffset(), cell.getRowLength(), row,
        roff, rlen);
      if (compare != 0) {
        return compare;
      }
    ...
    public byte[] getShortMidpointKey(final byte[] leftKey, final byte[] rightKey) {
      ...
      if (diffIdx >= minLength) {
        ...
        if ((0xff & diffByte) < 0xff
            && (diffByte + 1) < (rightKey[ROW_LENGTH_SIZE + diffIdx] & 0xff)) {
          newRowKey = new byte[diffIdx + 1];
  ...
  /**
   * HeapSize implementation We do not count the bytes in the rowCache because it should be empty
   * for a KeyValue in the MemStore.
   */
  @Override
  public long heapSize() {
     ...
     */
    return ClassSize.align(FIXED_OVERHEAD) + (offset == 0 ? ClassSize.sizeOfByteArray(length) // count
                                                                                              // both
                                                                                              // length
                                                                                              // and
                                                                                              // object
