...
2.  Enable secure communication on the Master node in _server-only_ mode with plaintext support.
...
3.  Enable secure communication on the Region Servers in both _server and client_ mode with plaintext support. Client mode here will ensure that RegionServer’s communication to Master is encrypted.
...
6.  Disable plaintext communication on the Region Servers by removing `supportplaintext` property. Restart RSs in rolling restart fashion.
