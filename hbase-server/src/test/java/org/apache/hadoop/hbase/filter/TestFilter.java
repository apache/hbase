...
public class TestFilter {
  ...
  public void setUp() throws Exception {
    TableDescriptor tableDescriptor =
        TableDescriptorBuilder.newBuilder(TableName.valueOf("TestFilter"))
            .setColumnFamily(ColumnFamilyDescriptorBuilder.newBuilder(FAMILIES[0])
                .setMinVersions(100).setMaxVersions(100).build())
  ...
  public void testWhileMatchFilterWithFilterRowWithReverseScan() throws Exception {
    ...
    while (true) {
      ...
      if (scannerCounter >= pageSize) {
        Assert.assertTrue("The WhileMatchFilter should now filter all remaining",
    ...
    scanner.close();
    Assert.assertEquals("The page filter returned more rows than expected", pageSize,
      scannerCounter);
  }
  ...
  public void testWhileMatchFilterWithFilterRow() throws Exception {
    ...
    while (true) {
      ...
      if (scannerCounter >= pageSize) {
        assertTrue("The WhileMatchFilter should now filter all remaining",
          filter.filterAllRemaining());
      }
      if (!isMoreResults) {
        break;
  ...
  public void testSingleColumnValueFilter() throws IOException {
    ...
    // Expect 3 rows (0, 2, 3)
    SingleColumnValueFilter scvf = new SingleColumnValueFilter(FAMILIES[0], QUALIFIERS_ONE[0],
        CompareOperator.EQUAL, VALUES[0]);
    ...
    // Expect 1 row (0)
    scvf = new SingleColumnValueFilter(FAMILIES[0], QUALIFIERS_ONE[0], CompareOperator.EQUAL,
        VALUES[0]);
    ...
    // Expect 1 row (3)
    scvf = new SingleColumnValueFilter(FAMILIES[0], QUALIFIERS_ONE[1], CompareOperator.EQUAL,
        VALUES[1]);
    scvf.setFilterIfMissing(true);
    s = new Scan().withStartRow(ROWS_THREE[0]).withStopRow(Bytes.toBytes("rowThree-4"));
    s.addFamily(FAMILIES[0]);
  ...
  public void testColumnValueFilter() throws Exception {
    ...
    KeyValue[] expectedMustPassOne =
        { new KeyValue(ROWS_ONE[0], FAMILIES[0], QUALIFIERS_FIVE[0], VALUES[1]),
            ...
            new KeyValue(ROWS_TWO[3], FAMILIES[1], QUALIFIERS_FIVE[1], VALUES[0]) }; // this pass
                                                                                     // scvf
