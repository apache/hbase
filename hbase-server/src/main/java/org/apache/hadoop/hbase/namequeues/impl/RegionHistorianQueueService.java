...
import org.apache.hadoop.hbase.namequeues.RegionHistorianPersistentService;
...
public class RegionHistorianQueueService implements NamedQueueService{
  ...
  private final boolean isRegionHistorianTableEnabled;
  private int queueSize;
  private final Queue<RegionHist.RegionHistorianPayload> regionHistorianQueue;
  private final RegionHistorianPersistentService regionHistorianPersistentService;
  ...
  public RegionHistorianQueueService(Configuration conf) {
    this.isRegionHistorianEnabled = conf.getBoolean(HConstants.REGION_HISTORIAN_BUFFER_ENABLED_KEY,
      HConstants.DEFAULT_REGION_HISTORIAN_SYS_TABLE_ENABLED_KEY);

    if(!isRegionHistorianEnabled) {
      this.isRegionHistorianTableEnabled = false;
      regionHistorianPersistentService = null;
      this.regionHistorianQueue = null;
      return;
    }
    ...
    this.isRegionHistorianTableEnabled = conf.getBoolean(HConstants.REGION_HISTORIAN_SYS_TABLE_ENABLED_KEY,
      HConstants.DEFAULT_REGION_HISTORIAN_SYS_TABLE_ENABLED_KEY);
    if (isRegionHistorianTableEnabled ) {
      regionHistorianPersistentService = new RegionHistorianPersistentService(conf);
    } else {
      regionHistorianPersistentService = null;
    }
  }
  ...
  synchronized void addToQueue(RegionHist.RegionHistorianPayload payload) {
    ...
    try {
      if (isRegionHistorianTableEnabled) {
        if (!payload.getRegionName().startsWith("hbase:regionHistorian")) {
          regionHistorianPersistentService.addToQueueForSysTable(payload);
        }
      }
    } catch (Exception e) {
      LOG.error("Error adding payload to queue for systable");
    }
  }
  ...
  public void persistAll(Connection connection) {
    ...
    if (regionHistorianPersistentService != null) {
      regionHistorianPersistentService.addAllLogsToSysTable(connection);
    }
  }
