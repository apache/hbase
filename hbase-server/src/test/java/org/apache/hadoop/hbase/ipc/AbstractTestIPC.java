...
public abstract class AbstractTestIPC {
  ...
  private void assertRpcAttribute(SpanData data, String methodName, InetSocketAddress addr,
    ...
    assertEquals(SERVICE.getDescriptorForType().getName(),
      data.getAttributes().get(SemanticAttributes.RPC_SERVICE_KEY));
    assertEquals(methodName, data.getAttributes().get(SemanticAttributes.RPC_METHOD_KEY));
    if (addr != null) {
      assertEquals(addr.getHostName(), data.getAttributes().get(SemanticAttributes.REMOTE_HOST_KEY));
      assertEquals(addr.getPort(), data.getAttributes().get(SemanticAttributes.REMOTE_PORT_KEY).intValue());
    }
