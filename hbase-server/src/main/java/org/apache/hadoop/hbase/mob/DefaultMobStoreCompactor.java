...
public class DefaultMobStoreCompactor extends DefaultCompactor {
  ...
  protected static final Logger LOG = LoggerFactory.getLogger(DefaultMobStoreCompactor.class);
  ...
  public List<Path> compact(CompactionRequestImpl request,
    ...
    LOG.debug("Mob compaction: major=" + request.isMajor() + " isAll=" + request.isAllFiles()
    ...
    LOG.debug("Mob compaction files: " + request.getFiles());
              ...
              if (mobCell.getCell().getValueLength() > mobSizeThreshold) {
                ...
                if (generationalMob) {
                  ...
                  if (stw != null) {
                    ...
                    // leave mob cell in a MOB file which is not in compaction selections
                  }
                } else {
  ...
  private List<Path> getReferencedMobFiles(Collection<HStoreFile> storeFiles) {
    Path mobDir = MobUtils.getMobFamilyPath(conf, store.getTableName(), store.getColumnFamilyName());
