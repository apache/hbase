...
public class TestOldestProcedureMetrics {
  ...
  public void testOldestProcedureAge() {
    ...
    when(procedureExecutor.getProcedures())
      .thenReturn(Arrays.asList(oldestLock, newerLock, serverCrash, finished));
    ...
    expected.put("LockProcedure", 9_000L);
    expected.put("ServerCrashProcedure", 8_000L);
    assertEquals(expected, wrapper.getOldestProcedureAgeByType());
    ...
    when(procedureExecutor.getProcedures()).thenReturn(Collections.emptyList());
    assertEquals(Collections.emptyMap(), wrapper.getOldestProcedureAgeByType());

    when(procedureExecutor.getProcedures()).thenReturn(Collections.singletonList(newerLock));
    when(newerLock.getSubmittedTime()).thenReturn(11_000L);
    assertEquals(Collections.singletonMap("LockProcedure", 0L),
