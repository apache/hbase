...
Use Maven Surefire 3.0.0 or later when possible, because it automatically aligns the
JUnit Platform Launcher with the JUnit version on the test runtime classpath. If you use
a version older than 3.0.0-M4, add a test dependency on the matching
`junit-platform-launcher`; otherwise test discovery may report `Tests run: 0`.
