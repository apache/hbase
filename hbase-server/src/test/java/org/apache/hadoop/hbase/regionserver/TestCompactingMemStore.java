...
public class TestCompactingMemStore extends TestDefaultMemStore {
  ...
  protected void compactingSetUp() throws Exception {
    ...
    this.region = HRegion.createHRegion(info, hbaseUtility.getDataTestDir(), conf, tableDescriptor,
      wal, true);
  ...
  public void testCompaction3Buckets() throws IOException {
    ...
    long totalHeapSize3 =
        totalHeapSize2 + CellArrayImmutableSegment.DEEP_OVERHEAD_CAM + 3 * oneCellOnCSLMHeapSize;
    ...
    assertEquals(
      4 * oneCellOnCAHeapSize + MutableSegment.DEEP_OVERHEAD
  ...
  public void testMagicCompaction3Buckets() throws IOException {
    ...
    memstore.getConfiguration()
        .setDouble(AdaptiveMemStoreCompactionStrategy.ADAPTIVE_COMPACTION_THRESHOLD_KEY, 0.45);
    memstore.getConfiguration()
        .setInt(AdaptiveMemStoreCompactionStrategy.COMPACTING_MEMSTORE_THRESHOLD_KEY, 2);
