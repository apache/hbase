...
public class HStoreFileToStoreFile extends Configured implements Tool {
  ...
  implements Mapper<LongWritable, Text, LongWritable, LongWritable> {
    private HBaseConfiguration c;

    @Override
    public void configure(JobConf job) {
      super.configure(job);
      c = new HBaseConfiguration(job);
    }

    @Override
    public void map(LongWritable key, Text value,
        OutputCollector<LongWritable, LongWritable> collector, Reporter reporter)
        throws IOException {
      Path p = new Path(value.toString());
      reporter.setStatus(key.toString() + " " + p.toString());
