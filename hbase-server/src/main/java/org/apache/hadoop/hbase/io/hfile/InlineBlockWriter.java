...
public interface InlineBlockWriter {
   ...
   * @param closing whether the file is being closed, in which case we need to write out all
   *          available data and not wait to accumulate another block
   */
   ...
   * {@link #shouldWriteBlock(boolean)} returned true.
   * @param out a stream (usually a compressing stream) to write the block to
   */
