...
public class TestSyncTable {
  ...
  private void assertEqualTables(int expectedRows, String sourceTableName, String targetTableName) 
      throws Exception {
    HTable sourceTable = new HTable(TEST_UTIL.getConfiguration(),
      TableName.valueOf(sourceTableName));
    HTable targetTable = new HTable(TEST_UTIL.getConfiguration(),
      TableName.valueOf(targetTableName));
  ...
  private void writeTestData(String sourceTableName, String targetTableName)
    ...
    for (; rowIndex < 40; rowIndex++) {
      ...
      sourcePut.add(family, column1, timestamp, value1);
      sourcePut.add(family, column2, timestamp, value2);
      sourceTable.put(sourcePut);
     
      Put targetPut = new Put(Bytes.toBytes(rowIndex));
      targetPut.add(family, column1, timestamp, value1);
      targetPut.add(family, column2, timestamp, value2);
    ...
    for (; rowIndex < 50; rowIndex++) {
      ...
      put.add(family, column1, timestamp, value1);
      put.add(family, column2, timestamp, value2);
      sourceTable.put(put);
    }
    ...
    for (; rowIndex < 90; rowIndex++) {
      Put sourcePut = new Put(Bytes.toBytes(rowIndex));
      sourcePut.add(family, column1, timestamp, column1);
      sourcePut.add(family, column2, timestamp, value2);
      sourceTable.put(sourcePut);

      Put targetPut = new Put(Bytes.toBytes(rowIndex));
      targetPut.add(family, column1, timestamp+1, column1);
      targetPut.add(family, column2, timestamp-1, value2);
      targetTable.put(targetPut);
    }
    ...
    for (; rowIndex < numRows; rowIndex++) {
      ...
      Put targetPut = new Put(Bytes.toBytes(rowIndex));
      targetPut.add(family, column1, timestamp, value3);
      targetPut.add(family, column2, timestamp, value3);
      targetTable.put(targetPut);
    }
