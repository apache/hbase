...
public interface RegionInfo extends Comparable<RegionInfo> {
  ...
  /**
   * to keep appended int's sorted in string format. Only allows 2 bytes to be sorted for replicaId.
   */
  @InterfaceAudience.Private
  ...
  @InterfaceAudience.Private
  Comparator<RegionInfo> COMPARATOR = (RegionInfo lhs, RegionInfo rhs) -> {
    if (rhs == null) {
      return 1;
    }
   ...
   * @return Region name made of passed tableName, startKey and id
   */
  static byte[] createRegionName(final TableName tableName, final byte[] startKey, final String id,
      boolean newFormat) {
  ...
  static byte[] createRegionName(final TableName tableName, final byte[] startKey,
    ...
    return createRegionName(tableName, startKey, Bytes.toBytes(Long.toString(regionid)), replicaId,
      newFormat);
  }
  ...
  static RegionInfo createMobRegionInfo(TableName tableName) {
    ...
    return new MutableRegionInfo(tableName, Bytes.toBytes(".mob"), HConstants.EMPTY_END_ROW, false,
        0, DEFAULT_REPLICA_ID, false);
  }
  ...
  static byte[][] parseRegionNameOrReturnNull(final byte[] regionName) {
    ...
    if (regionName.length > MD5_HEX_LENGTH + 2 && regionName[regionName.length - 1] == ENC_SEPARATOR
   ...
   * {@link RegionInfo#toByteArray(RegionInfo)} when writing to a stream and you want to use the pb
   * mergeDelimitedFrom (w/o the delimiter, pb reads to EOF which may not be what you want).
   * {@link #parseDelimitedFrom(byte[], int, int)} can be used to read back the instances.
   ...
   */
  static List<RegionInfo> parseDelimitedFrom(final byte[] bytes, final int offset, final int length)
      throws IOException {
    if (bytes == null) {
