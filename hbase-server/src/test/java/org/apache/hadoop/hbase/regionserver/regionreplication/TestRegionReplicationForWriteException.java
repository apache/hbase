...
public class TestRegionReplicationForWriteException {
  ...
  public void testWriteException() throws Exception {
    ...
    try {
      ...
      try {
        /**
         * Write First put,now the {@link WAL#sync} timeout and throws
         * {@link TimeoutIOException},{@link HRegion#put} is failed and rowKey1 is not applied to
        ...
        try (Table table = HTU.getConnection().getTable(tableName)) {
          assertFalse(checkReplica(table, FAMILY, QUAL, rowKey1, value1, 0));
        }
      ...
      try {
        ...
        /**
         * Write second put,now the {@link WAL#sync} is successful but
         * {@link RegionObserver#postBatchMutate} throws exception,the rowkey2 is applied to primary
         * and secondary replicas successfully.
         */
        try {
        ...
        try (Table table = HTU.getConnection().getTable(tableName)) {
          assertTrue(checkReplica(table, FAMILY, QUAL, rowKey2, value2, 0));
          HTU.waitFor(30000, () -> checkReplica(table, FAMILY, QUAL, rowKey2, value2, 1));
        }
      } finally {
        testCP = false;
      }
