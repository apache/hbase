...
public class TestOverallSkewChecker {
  ...
  public void testCheckSkew() {
    ...
    assertTrue(overallSkewChecker.needsSimplifiedBalance(tableLoads));
  }
  ...
  public void testCheckNotSkew() {
    ...
    assertFalse(overallSkewChecker.needsSimplifiedBalance(tableLoads));
  }

  private Map<TableName, Map<ServerName, List<RegionInfo>>> mockOneTableSkewClusterLoad(
  ...
  private Map<TableName, Map<ServerName, List<RegionInfo>>> mockTwoTableSkewClusterLoad(
    Map<String, Deque<BalancerRegionLoad>> loads) {
    Map<TableName, Map<ServerName, List<RegionInfo>>> allTableLoads = new HashMap<>();
    allTableLoads.put(t1, mockServerRegions(t1, 10, 5, 3, loads, 80000L));
  ...
  protected List<RegionInfo> mockTableRegions(TableName tableName, int count,
    ...
    for (int i = 0; i < count; i++) {
      RegionInfo mockRegion = mockRegionInfo(
        tableName.getNameAsString() + (baseRegionName == null ? "_region_" :
          baseRegionName) + i, tableName);
      regions.add(mockRegion);
      Deque<BalancerRegionLoad> BalancerRegionLoads =
        loads.computeIfAbsent(mockRegion.getEncodedName(), k -> new LinkedList<>());
