...
abstract class RegionReplicaGroupingCostFunction extends CostFunction {
  ...
  protected final long getMaxCost(BalancerClusterState cluster) {
    ...
    HashMap<Integer, HashSet<Integer>> primariesOfRegions =
      new HashMap<Integer, HashSet<Integer>>(cluster.numRegions);
   ...
   */
  protected final long costPerGroup(HashMap<Integer, HashSet<Integer>> primariesOfRegions) {
    long cost = 0;

    // primariesOfRegionsPerGroup is a hashmap of count of primary index on a server. a count > 1
    // means that replicas are co-hosted
    for(Map.Entry<Integer, HashSet<Integer>> pair : primariesOfRegions.entrySet()) {
