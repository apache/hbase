...
public class TestWithDisabledAuthorization extends SecureTestUtil {
  ...
  @RegisterExtension
  private TableNameTestExtension testTable = new TableNameTestExtension();
    ...
    verifyAllowed(new AccessTestAction() {
      ...
      public Object run() throws Exception {
        ...
        ACCESS_CONTROLLER.preModifyTable(ObserverContextImpl.createAndPrepare(CP_ENV),
          testTable.getTableName(), null, // not needed by AccessController
          tableDescriptor);
        return null;
      }
    }, SUPERUSER, USER_ADMIN, USER_RW, USER_RO, USER_OWNER, USER_CREATE, USER_QUAL, USER_NONE);
    ...
    verifyAllowed(new AccessTestAction() {
      ...
      public Object run() throws Exception {
        ACCESS_CONTROLLER.preCheckAndPut(ObserverContextImpl.createAndPrepare(RCP_ENV), TEST_ROW,
          TEST_FAMILY, TEST_Q1, CompareOperator.EQUAL, new BinaryComparator(Bytes.toBytes("foo")),
