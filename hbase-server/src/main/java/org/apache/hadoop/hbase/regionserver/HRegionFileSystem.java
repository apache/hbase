  ...
  public Path splitStoreFile(RegionInfo hri, String familyName, HStoreFile f, byte[] splitRow,
    ...
    if (createLinkFile) {
      ...
      try {
        ...
        CreateHFileLinkParams params = CreateHFileLinkParams.create(conf, fs, splitDir, familyName,
          hri.getTable().getNameAsString(), hri.getEncodedName(), linkedTable, linkedRegion,
          hfileName, true);
        tracker.createHFileLink(params);
        Path path =
