...
public class TestAsyncMetaRegionLocator {
  ...
  public void setUp() throws IOException {
    ...
    registry = ConnectionRegistryFactory.getRegistry(conf, User.getCurrent());
  ...
  public void test(TestInfo testInfo) {
    ...
    if (registry instanceof RpcConnectionRegistry) {
      ...
      // we need to fetch cluster id at the first time
      Matcher<SpanData> fetchClusterIdMatcher = allOf(
        hasName(endsWith("ConnectionRegistryService/GetConnectionRegistry")),
        hasParentSpanId(registryGetMetaRegionLocationsSpan), hasKind(SpanKind.CLIENT), hasEnded());
      assertThat(spans, hasItem(fetchClusterIdMatcher));
      SpanData fetchClusterIdSpan = spans.stream().filter(fetchClusterIdMatcher::matches)
        .findFirst().orElseThrow(AssertionError::new);
      Matcher<SpanData> clientGetMetaRegionLocationsMatcher =
        allOf(hasName(endsWith("ClientMetaService/GetMetaRegionLocations")),
          hasParentSpanId(fetchClusterIdSpan), hasKind(SpanKind.CLIENT), hasEnded());
