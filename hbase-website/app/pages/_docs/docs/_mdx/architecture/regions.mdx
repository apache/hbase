...
<Callout type="info">
  ...
  results](#major-compactions-change-query-results) for a more in-depth explanation. This situation
...
<Callout type="info" title="Legacy Information">
  ...
  Algorithm](#compaction.ratiobasedcompactionpolicy.algorithm). For information on the way that
  compactions work in HBase 0.96.x and later, see [Compaction](#compaction).
</Callout>
...
<Callout type="info" title="Impact of Key Configuration Options">
  ...
  Compaction Algorithm](#compaction.parameters).
</Callout>
