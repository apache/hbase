...
public class CompoundBloomFilterWriter extends CompoundBloomFilterBase
  ...
  /**
   * @param chunkByteSizeHint each chunk's size in bytes. The real chunk size might be different as
   *          required by the fold factor.
   * @param errorRate target false positive rate
   * @param hashType hash function type to use
   * @param maxFold maximum degree of folding allowed
   * @param bloomType the bloom type
   */
  ...
  private void enqueueReadyChunk(boolean closing) {
    ...
    if (firstKeyInChunk == null) {
      throw new NullPointerException(
          "Trying to enqueue a chunk, " + "but first key is null: closing=" + closing
              + ", keyCount=" + chunk.getKeyCount() + ", maxKeys=" + chunk.getMaxKeys());
    }
