...
<Callout type="info">
  A cluster whose promotion was blocked is still a replica — the read-only coprocessors remain loaded and
  writes are still rejected — even though its `hbase-site.xml` has `hbase.global.readonly.enabled=false`.
</Callout>
...
<Callout type="warning">
  **Do not remove the sentinel file while the cluster that wrote it is still running as an active cluster.**
  Doing so would allow two clusters to write to the same shared storage simultaneously, risking data
  corruption.
</Callout>
