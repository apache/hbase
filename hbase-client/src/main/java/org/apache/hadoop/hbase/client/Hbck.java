 ...
 * state, e.g. region not assigned, or some hdfs files left behind. After running any of the below,
 * operators may have to do some clean up on hdfs or schedule some assign procedures to get regions
 * back online. DO AT YOUR OWN RISK. For experienced users only.
...
public interface Hbck extends Abortable, Closeable {
  /**
   * Update table state in Meta only. No procedures are submitted to open/assign or close/unassign
   ...
   */
  Map<String, RegionState.State> setRegionStateInMeta(
   ...
   *          bypassed. When a Procedure has been bypassed, a Procedure will have completed but no
   *          other Procedure will be able to make progress on the target entity (intentionally).
   *          This override flag will override this fencing mechanism.
   * @param encodedRegionNames Region encoded names; e.g. 1588230740 is the hard-coded encoding for
   *          hbase:meta region and de00010733901a05f5a2a3a382e27dd4 is an example of what a random
   *          user-space encoded Region name looks like.
   */
  ...
  /**
   * Bypass specified procedure and move it to completion. Procedure is marked completed but no
   * actual work is done from the current state/step onwards. Parents of the procedure are also
   * marked for bypass.
   ...
   * @param recursive If set, if a parent procedure, we will find and bypass children and then the
   *          parent procedure (Dangerous but useful in case where child procedure has been 'lost').
