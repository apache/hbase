...
public class TestCleanerClearHFiles {
  ...
  private static final String DEFAULT_ARCHIVE_PREFIX = "data/default/";
  ...
  public void testClearArchive() throws Exception {
    ...
    Path archiveTable1Path = new Path(archiveDir, DEFAULT_ARCHIVE_PREFIX + TABLE1);
    Path archiveTable2Path = new Path(archiveDir, DEFAULT_ARCHIVE_PREFIX + TABLE2);

    TEST_UTIL.waitFor(10000,
      () -> !notExistOrEmptyDir(archiveTable1Path, fs) && !notExistOrEmptyDir(archiveTable2Path,
        fs));
