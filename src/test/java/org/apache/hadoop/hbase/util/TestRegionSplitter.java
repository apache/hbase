...
public class TestRegionSplitter {
  ...
  /**
   * Unit tests for the UniformSplit algorithm. Makes sure it divides up the
   * space of keys in the way that we expect.
   */
  @Test
  public void unitTestUniformSplit() {
    ...
    //      "Splitting into <2 regions should have thrown exception");
    ...
    byte[][] threeRegionsSplits = splitter.split(3);
    assertEquals(2, threeRegionsSplits.length);
    byte[] expectedSplit0 = new byte[] { 0x55, 0x55, 0x55, 0x55, 0x55, 0x55,
        0x55, 0x55 };
    assertArrayEquals(expectedSplit0, threeRegionsSplits[0]);
    byte[] expectedSplit1 = new byte[] { (byte) 0xAA, (byte) 0xAA, (byte) 0xAA,
        (byte) 0xAA, (byte) 0xAA, (byte) 0xAA, (byte) 0xAA, (byte) 0xAA };
    assertArrayEquals(expectedSplit1, threeRegionsSplits[1]);

    // Check splitting existing regions that have start and end points
    byte[] splitPoint = splitter
    ...
    splitPoint = splitter.split(new byte[] { (byte) 0xdf, xFF, xFF, xFF, xFF,
        xFF, xFF, xFF }, lastRow);
    assertArrayEquals(splitPoint, new byte[] { (byte) 0xef, xFF, xFF, xFF, xFF,
        xFF, xFF, xFF });
  }
  ...
  private void verifyBounds(List<byte[]> expectedBounds, String tableName)
    ...
    final Map<HRegionInfo, HServerAddress> regionInfoMap = hTable
        .getRegionsInfo();
    assertEquals(numRegions, regionInfoMap.size());
    for (Map.Entry<HRegionInfo, HServerAddress> entry : regionInfoMap
        .entrySet()) {
   ...
   * doesn't override equals(). This method checks whether a list contains a
   * given element by checking each element using the byte array comparator.
   *
   * @return the index of the first element that equals compareTo, or -1 if no
   *         elements are equal.
   */
