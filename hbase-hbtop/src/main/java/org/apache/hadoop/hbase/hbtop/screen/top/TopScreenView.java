...
public class TopScreenView extends AbstractScreenView {
  ...
  public ScreenView handleKeyPress(KeyEvent keyEvent) {
    ...
    switch (keyEvent.getCharacter()) {
      ...
      case 'f':
        cancelTimer();
        return topScreenPresenter.transitionToFieldScreen(screen, terminal);

      case 'm':
        cancelTimer();
        return topScreenPresenter.transitionToModeScreen(screen, terminal);

      case 'h':
        cancelTimer();
        return topScreenPresenter.transitionToHelpScreen(screen, terminal);

      case 'd':
        cancelTimer();
        return topScreenPresenter.goToInputModeForRefreshDelay(screen, terminal, MESSAGE_ROW);

      case 'o':
        cancelTimer();
        return topScreenPresenter.goToInputModeForFilter(screen, terminal, MESSAGE_ROW, true);

      case 'O':
        cancelTimer();
        return topScreenPresenter.goToInputModeForFilter(screen, terminal, MESSAGE_ROW, false);

      case 'p':
        cancelTimer();
        return topScreenPresenter.goToFilterDisplayMode(screen, terminal, MESSAGE_ROW);
  ...
  public TerminalSize getTerminalSize() {
    TerminalSize terminalSize = terminal.getSize();
    updatePageSize(terminalSize);
    return terminalSize;
  }

  public TerminalSize doResizeIfNecessary() {
    TerminalSize terminalSize = terminal.doResizeIfNecessary();
  ...
  private void showSummary(Summary summary) {
    TerminalPrinter printer = terminal.getTerminalPrinter(SUMMARY_START_ROW);
  ...
  private void showRecords(List<Header> headers, List<Record> records, Record selectedRecord) {
    TerminalPrinter printer = terminal.getTerminalPrinter(RECORD_START_ROW);
  ...
  private void showHeaders(List<Header> headers) {
    ...
    terminal.getTerminalPrinter(RECORD_HEADER_ROW).startHighlight()
      .print(header).stopHighlight().endOfLine();
  }
  ...
  private void clearMessage() {
    terminal.getTerminalPrinter(MESSAGE_ROW).print("").endOfLine();
  }

  private ScreenView unknownCommandMessage() {
    cancelTimer();
    return topScreenPresenter.goToMessageMode(screen, terminal, MESSAGE_ROW,
