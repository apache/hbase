...
See [arch.overview](#arch-overview) in the Architecture chapter.

### Does HBase support SQL?

Not really. SQL-ish support for HBase via [Hive](https://hive.apache.org/) is in development, however Hive is based on MapReduce which is not generally suitable for low-latency requests. See the [datamodel](#datamodel) section for examples on the HBase client.

### How can I find examples of NoSQL/HBase?

See the link to the BigTable paper in [other.info](#other-info), as well as the other papers.
...
See [datamodel](#datamodel) and [schema](#schema).
...
See [secondary.indexes](#secondary-indexes).

### Can I change a table's rowkeys?

This is a very common question. You can't. See [changing.rowkeys](#changing-rowkeys).

### What APIs does HBase support?

See [datamodel](#datamodel), [architecture.client](#architecture-client), and [external_apis](#external_apis).

## MapReduce

### How can I use MapReduce with HBase?

See [mapreduce](#mapreduce).
...
EC2 issues are a special case. See [trouble.ec2](#trouble-ec2) and [perf.ec2](#perf-ec2).
