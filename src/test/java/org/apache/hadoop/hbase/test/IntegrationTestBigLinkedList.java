...
public class IntegrationTestBigLinkedList extends IntegrationTestBase {
  ...
  static class Generator extends Configured implements Tool {
    ...
    static class GeneratorMapper
      ...
      protected void map(BytesWritable key, NullWritable value, Context output) throws IOException {
        current[curIteration] = new byte[key.getLength()];
        System.arraycopy(key.getBytes(), 0, current[curIteration], 0, key.getLength());
        if (++curIteration == current.length) {
    ...
    protected void createSchema(int shards) throws IOException {
      HBaseAdmin admin = new HBaseAdmin(getConf());
      StringBytes tableName = getTableName(getConf());
      if (!admin.tableExists(tableName)) {
        // TODO:(elliott) Add StringBytes constructor for HTD.
        HTableDescriptor htd = new HTableDescriptor(getTableName(getConf()).getBytes());
        htd.addFamily(new HColumnDescriptor(FAMILY_NAME));
        byte[] endRk = new byte[ROWKEY_LENGTH];

        for (int i = 0; i < endRk.length; i++) {
          endRk[i] = Byte.MAX_VALUE;
        }
        admin.createTable(htd, new byte[ROWKEY_LENGTH], endRk, shards);
      }
      admin.close();
    }
  ...
  static class Verify extends Configured implements Tool {
    ...
    public int run(Path outputDir, int numReducers) throws Exception {
      ...
      TableMapReduceUtil.initTableMapperJob(getTableName(getConf()).getString(), scan,
    ...
    public boolean verify(long expectedReferenced) throws Exception {
      ...
      if (!success) {
        ...
        StringBytes tableName = getTableName(conf);
  ...
  private static class Walker extends Configured implements Tool {
    ...
    private CINode getNode(byte[] row, HTable table, CINode node) throws IOException {
      Get get = new Get.Builder(row).addColumn(FAMILY_NAME, COLUMN_PREV).create();
      Result result = table.get(get);
      return getCINode(result, node);
    }
  }

  static StringBytes getTableName(Configuration conf) {
    return new StringBytes(conf.get(TABLE_NAME_KEY, DEFAULT_TABLE_NAME));
  }

  private static CINode getCINode(Result result, CINode node) {
    node.key = Bytes.copyOfByteArray(result.getRow());
    if (result.containsColumn(FAMILY_NAME, COLUMN_PREV)) {
      node.prev = Bytes.copyOfByteArray(result.getValue(FAMILY_NAME, COLUMN_PREV));
    } else {
      node.prev = NO_KEY;
    }
  ...
  public static void main(String[] args) throws Exception {
    new IntegrationTestBigLinkedList().doStaticMain(args);
  }
}
