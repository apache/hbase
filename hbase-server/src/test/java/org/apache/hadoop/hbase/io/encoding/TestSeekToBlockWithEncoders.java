...
public class TestSeekToBlockWithEncoders {
  ...
  public void testSeekingToBlockToANotAvailableKey() throws IOException {
    ...
    KeyValue kv5 = new KeyValue(Bytes.toBytes("bbbcd"), Bytes.toBytes("f1"), Bytes.toBytes("q1"),
      Bytes.toBytes("val"));
    sampleKv.add(kv5);
    KeyValue toSeek = new KeyValue(Bytes.toBytes("bbbce"), Bytes.toBytes("f1"), Bytes.toBytes("q1"),
      Bytes.toBytes("val"));
  ...
  public void testSeekToBlockWithDecreasingCommonPrefix() throws IOException {
    ...
    KeyValue kv1 = new KeyValue(Bytes.toBytes("row10aaa"), Bytes.toBytes("f1"), Bytes.toBytes("q1"),
      Bytes.toBytes("val"));
    sampleKv.add(kv1);
    KeyValue kv2 = new KeyValue(Bytes.toBytes("row10aaa"), Bytes.toBytes("f1"), Bytes.toBytes("q2"),
      Bytes.toBytes("val"));
    sampleKv.add(kv2);
    KeyValue kv3 = new KeyValue(Bytes.toBytes("row10aaa"), Bytes.toBytes("f1"), Bytes.toBytes("q3"),
      Bytes.toBytes("val"));
    sampleKv.add(kv3);
  ...
  public void testSeekToBlockWithDiffFamilyAndQualifer() throws IOException {
    ...
    KeyValue kv5 = new KeyValue(Bytes.toBytes("aac"), Bytes.toBytes("fam1"), Bytes.toBytes("q2"),
      Bytes.toBytes("val"));
    sampleKv.add(kv5);
    KeyValue toSeek = new KeyValue(Bytes.toBytes("aac"), Bytes.toBytes("fam2"), Bytes.toBytes("q2"),
      Bytes.toBytes("val"));
    seekToTheKey(kv5, sampleKv, toSeek);
  }
  ...
  private void seekToTheKey(KeyValue expected, List<KeyValue> kvs, ExtendedCell toSeek)
    ...
    for (DataBlockEncoding encoding : DataBlockEncoding.values()) {
      ...
      HFileContext meta = new HFileContextBuilder().withHBaseCheckSum(false).withIncludesMvcc(false)
        .withIncludesTags(false).withCompression(Compression.Algorithm.NONE).build();
      HFileBlockEncodingContext encodingContext =
        encoder.newDataBlockEncodingContext(conf, encoding, HFILEBLOCK_DUMMY_HEADER, meta);
      ByteBuffer encodedBuffer =
        TestDataBlockEncoders.encodeKeyValues(encoding, kvs, encodingContext, this.useOffheapData);
