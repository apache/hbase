...
public class StoreScanner extends NonReversedNonLazyKeyValueScanner
  ...
  private static boolean isOnlyLatestVersionScan(Scan scan) {
    // No need to check for Scan#getMaxVersions because live version files generated
    // by store file
    // writer retains max versions specified in ColumnFamilyDescriptor for the given
    // CF
   ...
   */
  @Override
  public boolean next(List<? super ExtendedCell> outResult, ScannerContext scannerContext) throws IOException {
      ...
      LOOP: do {
        ...
        switch (qcode) {
          ...
          case INCLUDE_AND_SEEK_NEXT_COL:
            ...
            if (f != null) {
              cell = (ExtendedCell) f.transformCell(cell);
            }
            this.countPerRow++;
  ...
  public void updateReaders(List<HStoreFile> sfs, List<KeyValueScanner> memStoreScanners)
    ...
    try {
      if (!closeLock.tryLock()) {
        ...
        // scanner
        // is not closed till the shipped() call is completed. Hence in that case if at
        // all
      ...
      // SEE HBASE-19468 where the flushed files are getting compacted even before a
      // scanner
      // calls next(). So its better we create scanners here rather than next() call.
      // Ensure
      ...
      // newly created
      // store files. In case of stream scanners this eager creation does not induce
      // performance
      // penalty because in scans (that uses stream scanners) the next() call is bound
      // to happen.
      List<KeyValueScanner> scanners = store.getScanners(sfs, cacheBlocks, get, usePread, isCompaction, matcher,
   ...
   */
  protected void checkScanOrder(ExtendedCell prevKV, ExtendedCell kv, CellComparator comparator)
      throws IOException {
