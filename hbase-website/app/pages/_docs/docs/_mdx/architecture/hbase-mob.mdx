...
[Configure the MOB Cache](#mob-cache-configure)), then configure specific columns to hold MOB data.
...
<Callout type="warn" title="Be mindful of total time to complete compaction of a region">
  ...
  hfiles](#mob-troubleshoot-cleaner-toonew). Failure to make the adjustments discussed there could
  lead to dataloss.
</Callout>
...
7. When the _mobrefs_ report shows that no more data is stored in the MOB system then you can safely alter the column family configuration so that the MOB feature is disabled.
   ```bash
