...
public class ProfileServlet extends HttpServlet {
      ...
      try {
        ...
        if (profilerLock.tryLock(lockTimeoutSecs, TimeUnit.SECONDS)) {
          try {
            ...
            // to avoid auto-refresh by ProfileOutputServlet, refreshDelay can be specified via
            // url param
            int refreshDelay = getInteger(req, "refreshDelay", 0);

            // instead of sending redirect, set auto-refresh so that browsers will refresh with
            // redirected url
  ...
  // Java 8+ has Process#isAlive, earlier versions do not
  private static Method isAliveMethod;
  static {
    try {
      isAliveMethod = Process.class.getDeclaredMethod("isAlive");
    } catch (Exception e) {
      isAliveMethod = null;
    }
  }
  ...
  private static boolean isAlive(Process process) {
    // Invoke Process#isAlive if we can
    if (isAliveMethod != null) {
      try {
        return (boolean) isAliveMethod.invoke(process);
      } catch (Exception e) {
        if (LOG.isTraceEnabled()) {
          LOG.trace("Failed to invoke Process#isAlive on " + process, e);
        }
        // fall through to alternative
      }
    }
    // We can still determine if the process is alive or not by calling Process#exitValue,
    // which will throw an exception if the process has not exited.
    try {
      int exitValue = process.exitValue();
      if (LOG.isTraceEnabled()) {
        LOG.trace("Process " + process + " is dead with exitValue " + exitValue);
      }
    } catch (IllegalThreadStateException e) {
      // The process is still alive.
      return true;
    }
    // As far as we can determine, the process is dead.
    return false;
  }
