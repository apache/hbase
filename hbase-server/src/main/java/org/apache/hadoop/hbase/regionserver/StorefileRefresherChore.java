...
public class StorefileRefresherChore extends ScheduledChore {
  ...
  public StorefileRefresherChore(int period, boolean onlyMetaRefresh, HRegionServer regionServer,
    ...
    this.hfileTtl = this.regionServer.getConfiguration()
      .getLong(TimeToLiveHFileCleaner.TTL_CONF_KEY, TimeToLiveHFileCleaner.DEFAULT_TTL);
  ...
  protected void chore() {
    for (Region r : regionServer.getOnlineRegionsLocalContext()) {
      if (!r.isReadOnly() || r.getTableDescriptor().isReadOnly()) {
        // skip checking for this region if it can accept writes or the contents of this table can not be modified
        continue;
      }
      ...
      try {
        ...
        // Store files have a TTL in the archive directory. If we fail to refresh for that long, we
        // stop serving reads
