...
public class TestCellFlatSet {
  ...
  public TestCellFlatSet(String chunkType) {
    ...
    if (chunkType.equals("NORMAL_CHUNKS")) {
      ...
      // chunkCreator with smaller chunk size, so only 3 cell-representations can accommodate a
      // chunk
      chunkCreator = ChunkCreator.initialize(SMALL_CHUNK_SIZE, false, globalMemStoreLimit, 0.2f,
  ...
  private CellChunkMap setUpCellChunkMap(boolean asc) {
    ...
    for (Cell kv : cellArray) {
      ...
      idxOffset = ByteBufferUtils.putInt(idxBuffer, idxOffset, dataChunk.getId()); // write data
                                                                                   // chunk id
  ...
  private CellChunkMap setUpJumboCellChunkMap(boolean asc) {
    ...
    Chunk dataJumboChunk = chunkCreator.getChunk(CompactingMemStore.IndexType.CHUNK_MAP,
      ChunkType.JUMBO_CHUNK, smallChunkSize);
    ...
    for (Cell kv : cellArray) {
      ...
      dataJumboChunk = chunkCreator.getChunk(CompactingMemStore.IndexType.CHUNK_MAP,
        ChunkType.JUMBO_CHUNK, smallChunkSize);
      assertTrue(dataJumboChunk.isJumbo());
