...
public class TestPrefetchAwareLoadBalancerCostFunctions extends StochasticBalancerTestBase {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestPrefetchAwareLoadBalancerCostFunctions.class);
  ...
  private PrefetchAwareLoadBalancer loadBalancer = new PrefetchAwareLoadBalancer();
  ...
  public void testVerifyPrefetchAwareSkewnessCostFunctionEnabled() {
    PrefetchAwareLoadBalancer lb = new PrefetchAwareLoadBalancer();
    lb.loadConf(conf);
    assertTrue(Arrays.asList(lb.getCostFunctionNames()).contains(
      PrefetchAwareLoadBalancer.PrefetchAwareRegionSkewnessCostFunction.class.getSimpleName()));
  }

  @Test
  public void testVerifyPrefetchAwareSkewnessCostFunctionDisabled() {
    conf.setFloat(
      PrefetchAwareLoadBalancer.PrefetchAwareRegionSkewnessCostFunction.REGION_COUNT_SKEW_COST_KEY,
      0.0f);

    PrefetchAwareLoadBalancer lb = new PrefetchAwareLoadBalancer();
    lb.loadConf(conf);

    assertFalse(Arrays.asList(lb.getCostFunctionNames()).contains(
      PrefetchAwareLoadBalancer.PrefetchAwareRegionSkewnessCostFunction.class.getSimpleName()));
  }
  ...
  public void testVerifyPrefetchCostFunctionEnabled() {
    ...
    assertTrue(Arrays.asList(lb.getCostFunctionNames())
      .contains(PrefetchAwareLoadBalancer.PrefetchCacheCostFunction.class.getSimpleName()));
  }
  ...
  public void testPrefetchCost() {
    conf.set(HConstants.PREFETCH_PERSISTENCE_PATH_KEY, "/tmp/prefetch.persistence");
    PrefetchAwareLoadBalancer.PrefetchCacheCostFunction costFunction =
      new PrefetchAwareLoadBalancer.PrefetchCacheCostFunction(conf);

    for (int test = 0; test < clusterRegionPrefetchMocks.length; test++) {
      int[][] clusterRegionLocations = clusterRegionPrefetchMocks[test];
      TestPrefetchAwareLoadBalancerCostFunctions.MockClusterForPrefetch cluster =
        new TestPrefetchAwareLoadBalancerCostFunctions.MockClusterForPrefetch(
