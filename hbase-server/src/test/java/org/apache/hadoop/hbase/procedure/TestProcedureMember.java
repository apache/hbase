...
public class TestProcedureMember {
  ...
  /**
   * Handle failures if a member's commit phase fails. NOTE: This is the core difference that makes
   * this different from traditional 2PC. In true 2PC the transaction is committed just before the
   * coordinator sends commit messages to the member. Members are then responsible for reading its
   * TX log. This implementation actually rolls back, and thus breaks the normal TX guarantees.
   */
  @Test
  ...
  /**
   * Handle Failures if a member's commit phase succeeds but notification to coordinator fails NOTE:
   * This is the core difference that makes this different from traditional 2PC. In true 2PC the
   * transaction is committed just before the coordinator sends commit messages to the member.
