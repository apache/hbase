 ...
 * randomly pick up a replica region for lookup. When clients receive RegionNotServedException from
 * region servers, it will add these region locations to the stale location cache. The stale cache
 * will be cleaned up periodically by a chore.
 * </p>
 * It follows a simple algorithm to choose a replica to go:
 * <ol>
 * <li>If there is no stale location entry for rows it looks up, it will randomly pick a replica
 * region to do lookup.</li>
...
class CatalogReplicaLoadBalanceSimpleSelector
  ...
  private final ConcurrentMap<TableName, ConcurrentNavigableMap<byte[], StaleLocationCacheEntry>> staleCache =
  ...
  /**
   * Select an random replica id. In case there is no replica region configured, return the primary
   * replica id.
   * @return Replica id
   */
  ...
  public int select(final TableName tablename, final byte[] row,
    ...
    if ((EnvironmentEdgeManager.currentTime()
        - entry.getValue().getTimestamp()) >= STALE_CACHE_TIMEOUT_IN_MILLISECONDS) {
