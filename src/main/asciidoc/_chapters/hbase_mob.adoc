...
the original cell. In addition to any tags originall written to HBase, the reference cell prepends
...
In this case the fist four bytes are `\x00\x02|\x94` which corresponds to the bytes
...
likely be stored in the designed MOB storage are for this specific table. However, the file could
...
Server will use a server side tag to optimize lookign at the mob and archive area of the correct
