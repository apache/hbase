...
public class TestByteRangeUtils {
  ...
  public void testNumEqualPrefixBytes() {
    assertEquals(0, ByteRangeUtils.numEqualPrefixBytes(new SimpleByteRange(new byte[] { 1, 2, 3 }),
      new SimpleByteRange(new byte[] { 4, 5, 6 }), 1));
    assertEquals(2, ByteRangeUtils.numEqualPrefixBytes(new SimpleByteRange(new byte[] { 1, 2, 3 }),
  ...
  public void testCopyToNewArrays() {
    ...
    assertArrayEquals(new byte[] { 1, 2, 3 },
      ByteRangeUtils
        .copyToNewArrays(new ArrayList<>(Arrays.asList(new SimpleByteRange(new byte[] { 1, 2, 3 }),
          new SimpleByteRange(new byte[] { 4, 5, 6 }))))
        .get(0));
  ...
  public void testFromArrays() {
    ...
    assertEquals(
      new ArrayList<>(Arrays.asList(new SimpleMutableByteRange(new byte[] { 1, 2, 3 }),
        new SimpleMutableByteRange(new byte[] { 4, 5, 6 }))),
      ByteRangeUtils.fromArrays(
        new ArrayList<>(Arrays.asList(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }))));
  }
}
