...
//    Thread.sleep(1000);
  ...
  public void testScanBackingStorage() throws Exception {
    ...
    Method method = RefreshMetaProcedure.class
      .getDeclaredMethod("scanBackingStorage", Connection.class);
    method.setAccessible(true);

    List<RegionInfo> fsRegions = (List<RegionInfo>) method.invoke(procedure, TEST_UTIL.getConnection());
  ...
  public void testHasBoundaryChanged() throws Exception {
    ...
    RegionInfo region1 = RegionInfoBuilder.newBuilder(tableName)
      .setStartKey(Bytes.toBytes("start1"))
      .setEndKey(Bytes.toBytes("end1"))
      .build();

    RegionInfo region2 = RegionInfoBuilder.newBuilder(tableName)
      .setStartKey(Bytes.toBytes("start2"))
      .setEndKey(Bytes.toBytes("end1"))
      .build();

    RegionInfo region3 = RegionInfoBuilder.newBuilder(tableName)
      .setStartKey(Bytes.toBytes("start1"))
      .setEndKey(Bytes.toBytes("end2"))
      .build();

    Method method = RefreshMetaProcedure.class
      .getDeclaredMethod("hasBoundaryChanged", RegionInfo.class, RegionInfo.class);
    method.setAccessible(true);

    assertTrue("Different start keys should have been detected",
      (Boolean) method.invoke(procedure, region1, region2));

    assertTrue("Different end keys should have been detected",
      (Boolean) method.invoke(procedure, region1, region3));

    assertFalse("Identical boundaries should not have been identified",
      (Boolean) method.invoke(procedure, region1, region1));
  }
}
