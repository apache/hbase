...
The following table summarizes the HBase community's recommendations for Java versions:

- ✅ indicates a recommended deployment combination with a base level of testing. The community is willing to help diagnose and address issues.
- ⚠️ indicates possible compatibility challenges. Review the available guidance before choosing this combination.
- ❌ indicates an unsupported combination. The Java version is deprecated by the HBase community, or the combination is known not to work.
- Treat any combination not listed in the table as ❌.
- Newer JDKs may have compatibility issues with older HBase releases that cannot be fixed within HBase's compatibility guarantees.
- Check the table notes for limitations, including build failures, test failures, and operational issues.
...
| HBase Version | JDK 6 | JDK 7 | JDK 8 | JDK 11 | JDK 17 |   JDK 21    |
| :-----------: | :---: | :---: | :---: | :----: | :----: | :---------: |
|   HBase 3.1   |  ❌   |  ❌   |  ❌   |   ❌   |   ✅   |     ✅      |
|   HBase 3.0   |  ❌   |  ❌   |  ❌   |   ❌   |   ✅   | ✅ (3.0.1+) |
|   HBase 2.7   |  ❌   |  ❌   |  ✅   |   ✅   |   ✅   |     ✅      |
...
<Callout type="warn">
Preliminary support for JDK 11 was introduced with HBase 2.3.0, and for JDK 17 with HBase 2.5.x. JDK 11 and JDK 17 have precommit and nightly coverage on applicable branches. We mark support as ✅ after running some integration tests with that JDK version and confirming that community members use it in production clusters.

For JDK 11/JDK 17 support in HBase, please refer to [HBASE-22972](https://issues.apache.org/jira/browse/HBASE-22972) and [HBASE-26038](https://issues.apache.org/jira/browse/HBASE-26038)

For JDK 11/JDK 17 support in Hadoop, which may also affect HBase, please refer to [HADOOP-15338](https://issues.apache.org/jira/browse/HADOOP-15338) and [HADOOP-17177](https://issues.apache.org/jira/browse/HADOOP-17177)

</Callout>

<Callout type="info" title="JDK 21 support">
See [HBASE-29546](https://issues.apache.org/jira/browse/HBASE-29546) for JDK 21 compatibility work and [HBASE-30354](https://issues.apache.org/jira/browse/HBASE-30354) for nightly coverage. HBase's precommit checks do not yet test JDK 21.

JDK 21 validation for HBase 2.5 remains incomplete.
