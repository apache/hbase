...
public class TestDefaultAtomicQuota {
  ...
  public static void setUpBeforeClass() throws Exception {
    ...
    TEST_UTIL.getConfiguration().setInt(QuotaUtil.QUOTA_DEFAULT_USER_MACHINE_ATOMIC_READ_SIZE,
  ...
  private void configureLenientThrottles() throws IOException {
    try (Admin admin = TEST_UTIL.getAdmin()) {
      admin.setQuota(QuotaSettingsFactory.throttleUser(getUserName(), ThrottleType.ATOMIC_READ_SIZE,
        HConstants.DEFAULT_BLOCKSIZE * 10000, TimeUnit.SECONDS));
      admin.setQuota(QuotaSettingsFactory.throttleUser(getUserName(),
        ThrottleType.ATOMIC_REQUEST_NUMBER, 100_000, TimeUnit.SECONDS));
    }
  }
