...
The RegionServer Grouping feature has been reimplemented. See section on RegionServer Grouping in [Operations and Management](/docs/ops_mgt) for more details.

The `hbase:namespace` table has been removed and fold into `hbase:meta`. See [Data Model](/docs/datamodel) for more details.

There is no special consideration upgrading to hbase-2.4.x from 2.3.x. And for earlier versions, just follow the [Upgrade from 2.0.x-2.2.x to 2.3+](#upgrade2.3) guide. In general, 2.2.x should be rolling upgradeable, for 2.1.x or 2.0.x, you will need to clear the [Upgrade from 2.0 or 2.1 to 2.2+](#upgrade2.2) hurdle first.

## Upgrade from 2.0.x-2.2.x to 2.3+

There is no special consideration upgrading to hbase-2.3.x from earlier versions. From 2.2.x, it should be rolling upgradeable. From 2.1.x or 2.0.x, you will need to clear the [Upgrade from 2.0 or 2.1 to 2.2+](#upgrade2.2) hurdle first.
...
To read about HBCK's replacement, see [Operations and Management](/docs/ops_mgt) documentation.
...
- hbase.config.read.zookeeper.config (see [upgrade2.0.zkconfig](#upgrade2.0.zkconfig) for migration details)
...
- hbase.balancer.tablesOnMaster wasn’t removed, strictly speaking, but its meaning has fundamentally changed and users should not set it. See the section [upgrade2.0.regions.on.master](#upgrade2.0.regions.on.master) for details.
- hbase.master.distributed.log.replay See the section [upgrade2.0.distributed.log.replay](#upgrade2.0.distributed.log.replay) for details
- hbase.regionserver.disallow.writes.when.recovering See the section [upgrade2.0.distributed.log.replay](#upgrade2.0.distributed.log.replay) for details
- hbase.regionserver.wal.logreplay.batch.size See the section [upgrade2.0.distributed.log.replay](#upgrade2.0.distributed.log.replay) for details
...
- hbase.client.retries.number is now set to 10. Previously it was 35. Downstream users are advised to use client timeouts as described in [Configuration](/docs/configuration#timeout-settings) instead.
- hbase.client.serverside.retries.multiplier is now set to 3. Previously it was 10. Downstream users are advised to use client timeouts as described in [Configuration](/docs/configuration#timeout-settings) instead.
...
For more information, see [Upgrading Coprocessors to 2.0](#upgrade2.0.coprocessors).
...
<Callout type="info">
  please read the [Changes of Note!](#_changes_of_note) section carefully before rolling upgrading.
...
If running Integration Tests under Kerberos, see [upgrade2.0.it.kerberos](#upgrade2.0.it.kerberos).
