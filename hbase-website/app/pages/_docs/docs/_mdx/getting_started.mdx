...
description: "Documentation for getting_started"
---
# Introduction
...
```
Connect to your running instance of HBase using the `hbase shell` command, located in the *<span className="path">bin/</span>* directory of your HBase install. In this example, some usage and version information that is printed when you start HBase Shell has been omitted. The HBase Shell prompt ends with a `>` character.
...
```
You can stop HBase the same way as in the [quickstart](#quickstart) procedure, using the *bin/stop-hbase.sh* command.
```
...
This quickstart assumes that each node is a virtual machine and that they are all on the same network. It builds upon the previous quickstart, [Pseudo-Distributed for Local Testing](#quickstart_pseudo), assuming that the system you configured in that procedure is now `node-a`. Stop HBase on `node-a` before continuing.
...
<div id="passwordless.ssh.quickstart" class="formalpara">
