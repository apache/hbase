...
public class RefreshMetaProcedure extends AbstractStateMachineTableProcedure<RefreshMetaState> {
  ...
  private Flow executePrepare() throws IOException {
    ...
    if (pendingMutations.isEmpty()) {
      ...
      LOG.info("Prepared {} mutations for meta table", pendingMutations.size());
  ...
  private Flow executeApply(MasterProcedureEnv env) throws ProcedureSuspendedException {
    try {
      if (pendingMutations != null && !pendingMutations.isEmpty()) {
        ...
        LOG.debug("RefreshMetaProcedure applied {} mutations to meta table", pendingMutations.size());
        currentRegions = null;
        latestRegions = null;
        pendingMutations = null;
        retryCounter = null;
      ...
      LOG.warn("Failed to apply mutations to meta table, suspend {}secs", backoff, ioe);
  ...
  /** Prepares mutations by comparing the current regions in hbase:meta with the latest regions from backing storage.
   ...
   */
  private List<Mutation> prepareMutations(Map<String, RegionInfo> currentMap, Map<String, RegionInfo> latestMap) throws IOException {
    ...
    for (Map.Entry<String, RegionInfo> entry : latestMap.entrySet()) {
      ...
      if (currentRegion == null) {
        ...
        LOG.debug("Adding the region to meta: {}", latestRegion.getRegionNameAsString());
      } else {
    ...
    for (Map.Entry<String, RegionInfo> entry : currentMap.entrySet()) {
      String regionId = entry.getKey();
      RegionInfo currentRegion = entry.getValue();
      if (!latestMap.containsKey(regionId)) {
        mutations.add(MetaTableAccessor.makeDeleteFromRegionInfo(currentRegion,
          EnvironmentEdgeManager.currentTime()));
        LOG.debug("Removing region from meta: {}", currentRegion.getRegionNameAsString());
      }
    }
  ...
  private void applyMutations(Connection connection, List<Mutation> mutations) throws IOException {
    ...
    for (int i = 0; i < chunks.size(); i++) {
      ...
      List<Put> puts = chunk.stream()
        .filter(m -> m instanceof Put)
        .map(m -> (Put) m)
        .collect(Collectors.toList());

      List<Delete> deletes = chunk.stream()
  ...
  List<RegionInfo> scanBackingStorage(Connection connection) throws IOException {
    ...
    FileStatus[] namespaceDirs = fs.listStatus(dataDir, path -> !path.getName().matches(HIDDEN_DIR_PATTERN));
  ...
  private List<RegionInfo> scanTablesInNamespace(FileSystem fs, Path namespacePath) throws IOException {
    ...
    return tableDirs.parallelStream()
      .filter(tableDir -> {
        if (TableName.isMetaTableName(CommonFSUtils.getTableName(tableDir))) {
          LOG.debug("Skipping META table in namespace {}", namespacePath.getName());
          return false;
        }
        return true;
      })
      .flatMap(tableDir -> {
        try {
          ...
          LOG.debug("Found {} regions in table {} in namespace {}",
            tableRegions.size(), tableDir.getName(), namespacePath.getName());
          return tableRegions.stream();
        } catch (IOException e) {
          LOG.warn("Failed to scan table directory: {} for namespace {}",
            tableDir, namespacePath.getName(), e);
          return Stream.empty();
        }
      }).toList();
  }
  ...
  protected synchronized boolean setTimeoutFailure(MasterProcedureEnv env) {
    setState(org.apache.hadoop.hbase.shaded.protobuf.generated.ProcedureProtos.ProcedureState.RUNNABLE);
