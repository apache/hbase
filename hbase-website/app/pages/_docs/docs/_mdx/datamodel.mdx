---
title: "Data Model"
description: "Understanding HBase's data model including tables, rows, columns, cells, and versioning."
---
...
See [version.delete](#delete) for more information on deleting versions of columns, and see [compaction](#compaction) for more information on compactions.
...
Delete markers are purged during the next major compaction of the store, unless the `KEEP_DELETED_CELLS` option is set in the column family (See [cf.keep.deleted](#cf-keep-deleted)).
...
<Callout>
This behavior represents a fix for an unexpected change that was introduced in HBase 0.94, and was fixed in [HBASE-10118](https://issues.apache.org/jira/browse/HBASE-10118).
...
column family descriptor; see [cf.keep.deleted](#cf-keep-deleted) for an example
...
the section above, [new.version.behavior](#optional-new-version-and-delete-behavior-in-hbase-200).
...
See [ACID Semantics](/acid-semantics.html).
