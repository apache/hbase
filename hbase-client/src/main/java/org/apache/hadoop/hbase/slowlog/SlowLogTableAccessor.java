...
public class SlowLogTableAccessor {
  ...
  public static void addSlowLogRecords(final List<TooSlowLog.SlowLogPayload> slowLogPayloads,
    ...
    for (TooSlowLog.SlowLogPayload slowLogPayload : slowLogPayloads) {
      ...
      final Put put =
          new Put(rowKey).setDurability(Durability.SKIP_WAL).setPriority(HConstants.NORMAL_QOS)
  ...
  /**
   * Create rowKey: currentTimeMillis APPEND slowLogPayload.hashcode Scan on slowlog table should
   * keep records with sorted order of time, however records added at the very same time
   * (currentTimeMillis) could be in random order.
