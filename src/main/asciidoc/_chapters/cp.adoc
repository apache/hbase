...
make use of the `AsyncTable` directly. And there is a lightweight
...
method to get an `AsyncConnection` from `Connection`.
====
...
protobuf for coprocessor on 2.5.0. And in 3.0.0, we removed all the dependency on non-shaded
protobuf so you need to reimplement your coprocessor to make use of the shaded protobuf
version provided in hbase-thirdparty. Please see the <<protobuf,protobuf>> section for
more details.
