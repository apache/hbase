...
public class HBaseAdmin implements Admin {
  ...
  public List<Boolean> clearRegionHistoriansResponses(@Nullable final Set<ServerName> serverNames)
    throws IOException {
    if (CollectionUtils.isEmpty(serverNames)) {
      return Collections.emptyList();
    }
    return serverNames.stream().map(serverName -> {
      try {
        return clearRegionHistoriansResponses(serverName);
      } catch (IOException e) {
        System.out.println(e.getCause()+" "+e.getMessage()+" "+e.getStackTrace().toString());
        throw new RuntimeException(e);
      }
    }).collect(Collectors.toList());
  }
  ...
  private List<LogEntry> getRegionHistorianResponses(
    Map<String, Object> filterParams, int limit) throws IOException {
    return executeCallable(
      ...
      protected List<LogEntry> rpcCall() throws Exception {
        HBaseRpcController controller = rpcControllerFactory.newController();
        HBaseProtos.LogRequest logRequest =
          RequestConverter.buildRegionHistorianResponseRequest(filterParams, limit);
        HBaseProtos.LogEntry logEntry = master.getLogEntries(controller, logRequest);
        return ProtobufUtil.toRegionHistorianPayloads(logEntry);
      }
    });
  }
  ...
  public List<LogEntry> getLogEntries(Set<ServerName> serverNames, String logType,
    ...
    } else if (logType.equals("REGION_HISTORIAN")) {
      if (ServerType.REGION_SERVER.equals(serverType)) {
        throw new IllegalArgumentException(
          "Region Historian logs are not maintained by HRegionServer");
      }
      return getRegionHistorianResponses(filterParams, limit);
  ...
  private Boolean clearRegionHistoriansResponses(final ServerName serverName) throws IOException {
    AdminService.BlockingInterface admin = this.connection.getAdmin(serverName);
    return executeCallable(new RpcRetryingCallable<Boolean>() {
      @Override
      protected Boolean rpcCall(int callTimeout) throws Exception {
        ...
        AdminProtos.ClearRegionHistorianResponses clearRegionHistorianResponses = admin
          .clearRegionHistoriansResponses(controller, RequestConverter.buildClearRegionHistorianResponseRequest());
        return ProtobufUtil.toClearRegionHistorianPayload(clearRegionHistorianResponses);
      }
    });
  }
