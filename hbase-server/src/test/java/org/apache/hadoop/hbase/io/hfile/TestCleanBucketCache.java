...
public class TestCleanBucketCache {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
    HBaseClassTestRule.forClass(TestCleanBucketCache.class);
  ...
  @Test
  public void testCleanBucketCacheRegionClose() throws Exception {
    ...
    long blocksCount = regionServingRS.getBlockCache().get().getBlockCount();
    assertNotEquals(0, blocksCount);

    HRegion r = regionServingRS.getRegions(tableRegionClose).get(0);
    r.close(false, false, true);
    ...
    Map<String, Integer> response = admin.uncacheStaleBlocks(regionServingRS.getServerName());

    long newCacheSize = regionServingRS.getBlockCache().get().getBlockCaches()[1].getCurrentSize();
    assertTrue(newCacheSize < newUsedCacheSize);
    response.values().forEach(value -> assertEquals(blocksCount, value.intValue()));
  }

  @Test
  public void testCleanBucketCacheRegionAvailable() throws Exception {
    // Write to table and flush
    TableName tableRegionClose = writeDataToTable();
    ...
    Admin admin = TEST_UTIL.getAdmin();
    Map<String, Integer> response = admin.uncacheStaleBlocks(regionServingRS.getServerName());
    assertEquals(regionServingRS.getBlockCache().get().getBlockCaches()[1].getCurrentSize(),
      oldUsedCacheSize);
    response.values().forEach(value -> assertEquals(0, value.intValue()));
  }
