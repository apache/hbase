...
export function parseProperties(xmlContent) {
  // Ignore commented XML examples so they do not create duplicate docs entries.
  const xmlWithoutComments = xmlContent.replace(/<!--[\s\S]*?-->/g, '');
  ...
  while ((match = propertyBlockRegex.exec(xmlWithoutComments)) !== null) {
