...
description: "Role-based access control (RBAC), visibility labels, and data encryption strategies for securing HBase data at rest and in transit."
---
...
<Steps>
<Step>
Enable HFile v3, by setting `hfile.format.version` to 3 in _hbase-site.xml_. This is the default for HBase 1.0 and newer.
...
<Step>
Enable SASL and Kerberos authentication for RPC and ZooKeeper, as described in [security.prerequisites](/docs/security/client-access#prerequisites) and [ZooKeeper](/docs/zookeeper).
</Step>
</Steps>
...
1.  As a prerequisite, perform the steps in [Procedure: Basic Server-Side Configuration.](/docs/security/data-access#procedure-basic-server-side-configuration).
2.  Install and configure the AccessController coprocessor, by setting the following properties in _hbase-site.xml_. These properties take a list of classes.
    ...
    <Callout type="info">
      ...
      both together, see [Security Configuration Example](/docs/security/example).
    </Callout>
    ```xml
    <property>
