...
public class TestCustomCellTieredCompactionPolicy {
  ...
  private HStoreFile createFile(RegionInfo regionInfo, Path file, long minValue, long maxValue, long size, int seqId,
    long ageInDisk) throws IOException {
    FileSystem fs = FileSystem.get(TEST_UTIL.getConfiguration());
    HRegionFileSystem regionFileSystem = new HRegionFileSystem(TEST_UTIL.getConfiguration(),fs,
      file, regionInfo);
    ...
    msf.setMetadataValue(TIERING_CELL_TIME_RANGE, TimeRangeTracker.toByteArray(timeRangeTracker));
    return msf;
  }
  ...
  private CustomCellDateTieredCompactionPolicy mockAndCreatePolicy(RegionInfo regionInfo) throws Exception {
  ...
  public void testGetCompactBoundariesForMajorOneOnEachSide() throws Exception {
    ...
    ArrayList<HStoreFile> files = new ArrayList<>();
    files.add(createFile(file, 0, 1, 1024, 0));
    files.add(createFile(file, EnvironmentEdgeManager.currentTime(), EnvironmentEdgeManager.currentTime(), 1024, 1));
    assertEquals(3,
      ((DateTieredCompactionRequest)policy.selectMajorCompaction(files)).getBoundaries().size());
  }
  ...
  private void testShouldPerformMajorCompaction(long min, long max, int numFiles,
    ...
    RegionInfo mockedRegionInfo = mockRegionInfo();
    CustomCellDateTieredCompactionPolicy policy = mockAndCreatePolicy(mockedRegionInfo);
    Path file = preparePath();
    ArrayList<HStoreFile> files = new ArrayList<>();
    ManualEnvironmentEdge timeMachine = new ManualEnvironmentEdge();
    EnvironmentEdgeManager.injectEdge(timeMachine);
    for(int i=0; i<numFiles; i++) {
      MockHStoreFile mockedSFile = (MockHStoreFile) createFile(mockedRegionInfo, file, min, max, 1024, 0, HConstants.DEFAULT_MAJOR_COMPACTION_PERIOD);
