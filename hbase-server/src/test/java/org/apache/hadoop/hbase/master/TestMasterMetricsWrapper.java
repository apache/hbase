...
public class TestMasterMetricsWrapper {
  ...
  public void testInfo() {
    ...
    while (TEST_UTIL.getHBaseCluster().getMaster().getServerManager().getOnlineServers()
        .size() == regionServerCount) {
      Threads.sleep(10);
    }
  ...
  public void testOfflineRegion() throws Exception {
    ...
    try {
      ...
      while (true) {
        if (regionStates.getRegionByStateOfTable(table).get(RegionState.State.OFFLINE)
            .contains(hri)) {
          break;
        }
        long now = EnvironmentEdgeManager.currentTime();
