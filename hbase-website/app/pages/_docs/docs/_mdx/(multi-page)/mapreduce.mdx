---
title: "HBase and MapReduce"
description: "Guide to using Apache HBase with MapReduce including configuration, examples, and best practices."
---
...
<Callout type="info" title="mapred and mapreduce">
  ...
  and _org.apache.hadoop.hbase.mapreduce_. The former does old-style API and the latter the new
  ...
  _org.apache.hadoop.hbase.mapreduce_. In the notes below, we refer to _o.a.h.h.mapreduce_ but
  replace with _o.a.h.h.mapred_ if that is what you are using.
</Callout>
...
To give the MapReduce jobs the access they need, you could add _hbase-site.xml_to _\$HADOOP*HOME/conf* and add HBase jars to the _\$HADOOP_HOME/lib_ directory.
...
## Speculative Execution [#mapreduce-speculative-execution]
