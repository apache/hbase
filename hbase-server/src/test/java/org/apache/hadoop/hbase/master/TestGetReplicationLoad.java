...
public class TestGetReplicationLoad {
  ...
  public static void startCluster() throws Exception {
    ...
    StartTestingClusterOption option =
        StartTestingClusterOption.builder().masterClass(TestMasterMetrics.MyMaster.class).build();
  ...
  public void testGetReplicationMetrics() throws Exception {
    ...
    long ageOfLastShippedOp = 2, replicationLag = 3, timeStampOfLastShippedOp = 4,
        timeStampOfNextToReplicate = 5, editsRead = 6, oPsShipped = 7;
    ...
    ClusterStatusProtos.ReplicationLoadSource rload1 = ClusterStatusProtos.ReplicationLoadSource
        ...
        .setSizeOfLogQueue(sizeOfLogQueue).setTimeStampOfNextToReplicate(timeStampOfNextToReplicate)
        .setQueueId(queueId).setEditsRead(editsRead).setOPsShipped(oPsShipped).setRunning(running)
        .setRecovered(recovered).setEditsSinceRestart(editsSinceRestart).build();
    ClusterStatusProtos.ReplicationLoadSource rload2 =
        ClusterStatusProtos.ReplicationLoadSource.newBuilder().setPeerID(peer2)
            ...
            .setTimeStampOfNextToReplicate(timeStampOfNextToReplicate + 1).setQueueId(queueId)
            .setEditsRead(editsRead + 1).setOPsShipped(oPsShipped + 1).setRunning(running)
            .setRecovered(recovered).setEditsSinceRestart(editsSinceRestart).build();
