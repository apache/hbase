...
#### Server-Side Configuration [#security-data-access-acl-server-side-configuration]
...
Install and configure the AccessController coprocessor, by setting the following properties in _hbase-site.xml_. These properties take a list of classes.
...
Set up the Hadoop group mapper in the Hadoop namenode's _core-site.xml_. This is a Hadoop file, not an HBase file. Customize it to your site's needs. Following is an example.
...
### Server-Side Configuration [#security-data-access-visibility-labels-server-side-configuration]
...
### Server-Side Configuration [#security-data-access-transparent-encryption-of-data-at-test-server-side-configuration]
