 ...
 * <p>
 * To delete specific families, execute {@link #addFamily(byte[]) deleteFamily} for each family to
 * delete.
 * <p>
 * To delete multiple versions of specific columns, execute {@link #addColumns(byte[], byte[])
 * deleteColumns} for each column to delete.
 * <p>
 * To delete specific versions of specific columns, execute {@link #addColumn(byte[], byte[], long)
 * deleteColumn} for each column version to delete.
 * <p>
 ...
 * timestamp of 'now' where 'now' is the servers's System.currentTimeMillis(). Specifying a
 * timestamp to the deleteColumn method will delete versions only with a timestamp equal to that
...
public class Delete extends Mutation {
   ...
   *             {@link Delete#Delete(byte[], long, NavigableMap)} instead
   */
  @Deprecated
  @Override
