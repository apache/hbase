...
public class TestRegionCoprocessorQuotaUsage {
  ...
  public static class MyRegionObserver implements RegionObserver {
    @Override public void preGetOp(ObserverContext<RegionCoprocessorEnvironment> c, Get get,
      ...
      if (!THROTTLING_OCCURRED.get()) {
        try {
          c.getEnvironment()
            .checkBatchQuota(c.getEnvironment().getRegion(), OperationQuota.OperationType.GET);
