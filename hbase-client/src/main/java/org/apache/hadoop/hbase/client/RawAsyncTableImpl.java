...
class RawAsyncTableImpl implements AsyncTable<AdvancedScanResultConsumer> {
    ...
    if (
      ...
      return RawAsyncTableImpl.this
        .<CheckAndMutateResult> newCaller(checkAndMutate.getRow(), mutation.getPriority(),
          rpcTimeoutNs)
        .action((controller, loc, stub) -> RawAsyncTableImpl.mutate(controller, loc, stub, mutation,
  ...
  public CompletableFuture<Result> mutateRow(RowMutations mutations) {
    ...
    return this
      ...
      .action(
        (controller, loc, stub) -> this.<Result, Result> mutateRow(controller, loc, stub, mutations,
          (rn, rm) -> RequestConverter.buildMultiRequest(rn, rm, nonceGroup, nonce), resp -> resp))
      .call();
  }
