...
public class InputModeScreenView extends AbstractScreenView {
  ...
  public ScreenView handleKeyPress(KeyEvent keyEvent) {
    ...
    switch (keyEvent.getType()) {
      ...
      case Character:
        inputModeScreenPresenter.character(keyEvent.getCharacter());
        break;
  ...
  public void showInput(String message, String inputString, int cursorPosition) {
    terminal.getTerminalPrinter(row).startBold().print(message)
      .stopBold().print(" ").print(inputString).endOfLine();
    terminal.setCursorPosition(
      new CursorPosition(message.length() + 1 + cursorPosition, row));
    terminal.refresh();
  }
}
