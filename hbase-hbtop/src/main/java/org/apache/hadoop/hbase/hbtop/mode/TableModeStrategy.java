...
import org.apache.hadoop.hbase.hbtop.Filter;
...
public final class TableModeStrategy implements ModeStrategy {
  ...
  public List<Record> getRecords(ClusterMetrics clusterMetrics) {
    ...
    List<Record> records = regionModeStrategy.getRecords(clusterMetrics).stream()
      ...
      .peek(r -> r.put(Field.REGION_COUNT, 1))
  ...
  public DrillDownInfo drillDown(Record selectedRecord) {
    List<Filter> initialFilters = Arrays.asList(
      Filter.newBuilder(Field.NAMESPACE).doubleEquals(selectedRecord.get(Field.NAMESPACE)),
      Filter.newBuilder(Field.TABLE).doubleEquals(selectedRecord.get(Field.TABLE)));
