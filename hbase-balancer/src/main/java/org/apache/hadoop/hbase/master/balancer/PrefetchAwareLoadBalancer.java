 ...
 * where it maintains the amount by which a region is prefetched on a region server. During balancer
 * run, a region plan is generated that takes into account this prefetch information and tries to
...
public class PrefetchAwareLoadBalancer extends StochasticLoadBalancer {
  private static final Logger LOG = LoggerFactory.getLogger(PrefetchAwareLoadBalancer.class);
  ...
  protected List<CandidateGenerator> createCandidateGenerators() {
    ...
    candidateGenerators.add(GeneratorFunctionType.LOAD.ordinal(),
      new PrefetchBasedSkewnessCandidateGenerator());
    candidateGenerators.add(GeneratorFunctionType.PREFETCH.ordinal(),
      new PrefetchCandidateGenerator());
  ...
  protected List<CostFunction> createCostFunctions(Configuration configuration) {
    ...
    addCostFunction(costFunctions, new PrefetchAwareRegionSkewnessCostFunction(configuration));
    addCostFunction(costFunctions, new PrefetchCacheCostFunction(configuration));
  ...
  private synchronized void updateRegionLoad() {
    ...
    // Build olf region prefetch statistics
    clusterStatus.getLiveServerMetrics().forEach((ServerName sn, ServerMetrics sm) -> {
      ...
      sm.getRegionCachedInfo().forEach((String regionEncodedName, Integer prefetchSize) -> {
        // If the region is fond in currentPrefetchMap, it is currently hosted on this server
        if (currentPrefetchMap.containsKey(regionEncodedName)) {
          ...
          if (!ServerName.isSameAddress(currentServer, sn)) {
            ...
            float regionPrefetchRatio =
              regionSizeMB == 0 ? 0.0f : (float) prefetchSize / regionSizeMB;
  ...
  private class PrefetchCandidateGenerator extends CandidateGenerator {
    ...
    protected BalanceAction generate(BalancerClusterState cluster) {
      // Move the regions to the servers they were previously hosted on based on the prefetch ratio
      if (!oldRegionPrefetchMap.isEmpty() && oldRegionPrefetchMap.entrySet().iterator().hasNext()) {
        ...
        int oldServerIndex = cluster.serversToIndex
          .get(oldRegionPrefetchMap.get(regionEncodedName).getFirst().getAddress());
        if (oldServerIndex < 0) {
        ...
        float oldPrefetchRatio =
          cluster.getOrComputeRegionPrefetchRatio(regionIndex, oldServerIndex);
    ...
    private BalanceAction generatePlan(BalancerClusterState cluster, int regionIndex,
      int currentServerIndex, float currentServerPrefetchRatio, int oldServerIndex,
      float oldServerPrefetchRatio) {
      return moveRegionToOldServer(cluster, regionIndex, currentServerIndex,
        currentServerPrefetchRatio, oldServerIndex, oldServerPrefetchRatio)
    ...
    private boolean moveRegionToOldServer(BalancerClusterState cluster, int regionIndex,
      ...
      if (oldServerPrefetchRatio == 1.0f) {
        if (LOG.isDebugEnabled()) {
          LOG.debug("Region {} moved to the old server {} as it is fully prefetched there",
            cluster.regions[regionIndex].getEncodedName(), cluster.servers[oldServerIndex]);
        }
        return true;
      }

      // Move the region back to the old server if it is prefetched equally on both the servers
      if (currentServerPrefetchRatio == oldServerPrefetchRatio) {
        if (LOG.isDebugEnabled()) {
          LOG.debug("Region {} moved from {} to {} as the prefetch {} is same on both servers",
            cluster.regions[regionIndex].getEncodedName(), cluster.servers[currentServerIndex],
            cluster.servers[oldServerIndex], currentServerPrefetchRatio);
        }
        return true;
      }

      // If the region is not fully prefetched on either of the servers, move the region back to the
      // old server if the current server prefetch is still much less than the old server
      if (
        oldServerPrefetchRatio > 0.0f
          && currentServerPrefetchRatio / oldServerPrefetchRatio < prefetchDiffThreshold
      ) {
        if (LOG.isDebugEnabled()) {
          LOG.debug(
            "Region {} moved from {} to {} old prefetch {} is better than the current "
              + "prefetch {}",
            cluster.regions[regionIndex].getEncodedName(), cluster.servers[currentServerIndex],
            cluster.servers[oldServerIndex], currentServerPrefetchRatio, oldServerPrefetchRatio);
        }
        return true;
      }

      if (LOG.isDebugEnabled()) {
        LOG.debug("Region {} not moved from {} to {} with current prefetch {} and old prefetch {}",
  ...
  private class PrefetchBasedSkewnessCandidateGenerator extends LoadCandidateGenerator {
    ...
    BalanceAction pickRandomRegions(BalancerClusterState cluster, int thisServer, int otherServer) {
      ...
      if (!oldRegionPrefetchMap.isEmpty() && oldRegionPrefetchMap.entrySet().iterator().hasNext()) {
        ...
        if (otherServer < 0) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
              "PrefetchBasedSkewnessCandidateGenerator: Region {} not moved to the old "
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
            "PrefetchBasedSkewnessCandidateGenerator: Region {} moved from {} to {} as it "
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
          "PrefetchBasedSkewnessCandidateGenerator: Region {} moved from {} to {} as it is "
            + "least prefetched on current server",
    ...
    private int pickLeastPrefetchedRegion(BalancerClusterState cluster, int thisServer) {
      ...
      for (int i = 0; i < cluster.regionsPerServer[thisServer].length; i++) {
        ...
        float currentPrefetchRatio =
          cluster.getOrComputeRegionPrefetchRatio(regionIndex, thisServer);
    ...
    protected void regionMoved(int region, int oldServer, int newServer) {
      ...
      if (LOG.isDebugEnabled() && (prefetchRatio < 0.0 || prefetchRatio > 1.0)) {
        LOG.debug(
          ...
          cluster.servers[newServer].getHostname(), oldServerPrefetch, newServerPrefetch,
