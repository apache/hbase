...
module Hbase
  class EncryptedTableKeymetaTest < Test::Unit::TestCase
    ...
    define_test 'Test table put/get with encryption' do
      ...
      reader = HFile.createReader(fs, storeFileInfo.getPath(), CacheConfig::DISABLED, true,
      ...
      assert_raises(CorruptHFileException) do |e|
        reader = HFile.createReader(fs, storeFileInfo.getPath(), CacheConfig::DISABLED, true, conf)
        assert_true(e.message.include?(
          "Problem reading HFile Trailer from file #{storeFileInfo.getPath()}"))
      end
      Encryption.clearKeyProviderCache()
