      ...
      for (Iterator<Cell> it : codecIterators) {
        ...
        if (codecKv == null
          ...
          if (codecKv == null) {
            ...
            while (prefix < limitLength && codecKv.getBuffer()[prefix
                + codecKv.getOffset()] == currentKv.getBuffer()[prefix + currentKv.getOffset()]) {
              prefix++;
            }
  ...
  public void benchmarkAlgorithm(Compression.Algorithm algorithm, String name, byte[] buffer,
    ...
    try {
      ...
      throw new RuntimeException(
          String.format("Benchmark, or encoding algorithm '%s' cause some stream problems", name),
          e);
    }
