...
public class TestHFileOutputFormat2 {
  ...
  private void doIncrementalLoadTest(boolean shouldChangeRegions, boolean shouldKeepLocality,
    ...
    StartMiniClusterOption option = StartMiniClusterOption.builder().numRegionServers(hostCount)
        .dataNodeHosts(hostnames).build();
    ...
    try {
      ...
      if (shouldChangeRegions) {
        ...
        while (util.getConnection().getRegionLocator(chosenTable.getName()).getAllRegionLocations()
  ...
  public void testBlockStoragePolicy() throws Exception {
    ...
    conf.set(
      HFileOutputFormat2.STORAGE_POLICY_PROPERTY_CF_PREFIX + Bytes.toString(
        HFileOutputFormat2.combineTableNameSuffix(TABLE_NAMES[0].getName(), FAMILIES[0])),
      "ONE_SSD");
