...
public class QuotaObserverChore extends ScheduledChore {
  ...
  QuotaObserverChore(Connection conn, Configuration conf,
      SpaceQuotaSnapshotNotifier snapshotNotifier, MasterQuotaManager quotaManager,
      Stoppable stopper, MetricsMaster metrics) {
    ...
    this.regionReportLifetimeMillis = conf.getLong(REGION_REPORT_RETENTION_DURATION_KEY,
  ...
  static TimeUnit getTimeUnit(Configuration conf) {
    return TimeUnit.valueOf(
      conf.get(QUOTA_OBSERVER_CHORE_TIMEUNIT_KEY, QUOTA_OBSERVER_CHORE_TIMEUNIT_DEFAULT));
  }
