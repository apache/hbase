...
<Callout title="`mapred` and `mapreduce`">
...
See [HBase, MapReduce, and the CLASSPATH](#hbasemapredceclasspath) for current recommendations for resolving classpath errors.
...
For more on how this mechanism works, see [Bulk Load](#archbulkload).
...
If you have classpath errors, see [HBase, MapReduce, and the CLASSPATH](#hbasemapredceclasspath).
...
See [Speculative Execution](#specex) for more information.
