...
description: "HBase support for storing medium-sized binary objects (100KB-10MB) with optimized performance."
---
...
100KB and 10MB (see the [faq](#faq)). HBase 2 added special internal handling of MOBs
...
To take advantage of MOB, you need to use [HFile version 3](#hfilev3). Optionally,
configure the MOB file reader's cache settings for each RegionServer (see
[mob.cache.configure](#mob-cache-configure)), then configure specific columns to hold MOB data.
...
"Write Amplification" = log_K(M/S) where **K** is the number of files in compaction
...
below [mob.troubleshoot.cleaner.toonew](#mob-troubleshoot-cleaner-toonew). Failure to make the adjustments discussed there could
lead to dataloss.
</Callout>
