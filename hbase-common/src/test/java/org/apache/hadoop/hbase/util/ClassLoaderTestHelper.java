...
public final class ClassLoaderTestHelper {
  ...
  /**
   * Create a test jar for testing purpose for a given class name with specified code string: save
   * the class to a file, compile it, and jar it up. If the code string passed in is null, a bare
   * empty class will be created and used.
   * @param testDir   the folder under which to store the test class and jar
   * @param className the test class name
   * @param code      the optional test class code, which can be null. If null, a bare empty class
   *                  will be used
   ...
   * @param className the test class name
   * @param code      the optional test class code, which can be null. If null, an empty class will
   *                  be used
   * @param folder    the folder under which to store the generated jar
   * @return the test jar file generated
   */
  public static File buildJar(String testDir, String className, String code, String folder)
    throws Exception {
    ...
    if (
      !createJarArchive(jarFile, new File[] { new File(srcDir.toString(), className + ".class") })
    ) {
  ...
  /**
   * Add a list of jar files to another jar file under a specific folder. It is used to generated
   * coprocessor jar files which can be loaded by the coprocessor class loader. It is for testing
   * usage only so we don't be so careful about stream closing in case any exception.
