---
title: "RegionServer Off-Heap Read/Write Path"
---
...
![Offheap Overview](../images/offheap-overview.png)

For redress for the single mention of onheap in the read-section of the diagram above see [Read block from HDFS to offheap directly](#regionserver-read-hdfs-block-offheap).
...
For an end-to-end off-heaped read-path, all you have to do is enable an off-heap backed [Off-heap Block Cache](#offheap-blockcache)(BC).
To do this, configure _hbase.bucketcache.ioengine_ to be _offheap_ in _hbase-site.xml_ (See [BucketCache Deploy Modes](#bc-deploy-modes) to learn
more about _hbase.bucketcache.ioengine_ options). Also specify the total capacity of the BC using `hbase.bucketcache.size`.
Please remember to adjust value of 'HBASE_OFFHEAPSIZE' in _hbase-env.sh_ (See [BucketCache Example Configuration](#bc-example) for help sizing and an example
...
such as Server RPC buffer pool and short-circuit reads (See discussion in [BucketCache Example Configuration](#bc-example)).

Please keep in mind that there is no default for `hbase.bucketcache.ioengine` which means the `BlockCache` is OFF by default
(See [Direct Memory Usage In HBase](#direct-memory)). 

This is all you need to do to enable off-heap read path. Most buffers in HBase are already off-heap. With BC off-heap,
the read pipeline will copy data between HDFS and the server socket — caveat [hbase.ipc.server.reservoir.initial.max](#hbase-ipc-server-reservoir-initial-max) —
...
<Callout type="info">
The config keys which start with prefix `hbase.ipc.server.reservoir` are deprecated in hbase-3.x (the
...
(See [deprecated and new configs in HBase3.x](#regionserver-read-hdfs-block-offheap))
</Callout>
...
hbase-2.2.x configurations discussed above in the [Tuning the RPC buffer pool](#regionserver-offheap-rpc-bb-tuning). Much of the advice here overlaps that given above
in the [Tuning the RPC buffer pool](#regionserver-offheap-rpc-bb-tuning) since the implementations have similar configurations.
...
(similar to the discussion above in [Tuning the RPC buffer pool](#regionserver-offheap-rpc-bb-tuning)). If the default `hbase.regionserver.handler.count` is 30, then the default will be 1890.
...
![ByteBuff Allocator Stats](../images/bytebuff-allocator-stats.png)
...
Don't forget to up your _HBASE_OFFHEAPSIZE_ accordingly. See [hbase.offheapsize](#hbase-offheapsize)
...
MaxDirectMemorySize property (see [hbase.offheapsize](#hbase-offheapsize) for more on _HBASE_OFFHEAPSIZE_). The default value of
