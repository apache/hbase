  ...
  protected class ClusterID {
    ...
    public HConnection getConnection() throws Exception {
      if (this.connection == null) {
        this.connection = HConnectionManager.createConnection(this.configuration);
      }
  ...
  protected class VerifyReplicationLoop extends Configured implements Tool {
    ...
    protected void setupTablesAndReplication() throws Exception {
      ...
      for (ClusterID cluster : clusters) {
        HBaseAdmin admin = new HBaseAdmin(cluster.getConnection());
        try {
          if (admin.tableExists(tableName)) {
             ...
             */
            Set<ServerName> regionServers = new TreeSet<ServerName>();
            for (HRegionLocation location: admin.getConnection().locateRegions(tableName)) {
              regionServers.add(location.getServerName());
            }
            for (ServerName server : regionServers) {
              source.getConnection().getAdmin(server).rollWALWriter(null,
      ...
      if (!source.equals(sink)) {
        ...
        HashMap<TableName, ArrayList<String>> toReplicate =
            new HashMap<TableName, ArrayList<String>>();
