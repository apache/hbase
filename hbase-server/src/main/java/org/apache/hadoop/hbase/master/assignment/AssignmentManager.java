...
public class AssignmentManager {
  ...
  public static final String PROCESS_UNKNOWN_RS_ON_STARTUP = "hbase.master.assign.regions.on.unknown.servers";
  ...
  public void processRegionsOnUnknownServers() {
    if (!getConfiguration().getBoolean(PROCESS_UNKNOWN_RS_ON_STARTUP, DEFAULT_PROCESS_UNKNOWN_RS_ON_STARTUP)) {
      LOG.info("Not attempting to assign any regions on UNKNOWN RegionServers because of configuration.");
      return;
    }
    LOG.info("Reading meta to schedule process any UNKNOWN RegionServers without pending ServerCrashProcedure.");
    AtomicInteger unassignedRegions = new AtomicInteger(0);
    final List<Procedure<MasterProcedureEnv>> currProcedures = master.getMasterProcedureExecutor().getProcedures();
    Set<ServerName> unknownServers = regionStates.getRegionStates().stream().filter(s -> {
      ...
      return master.getServerManager().isServerKnownAndOnline(serverName).equals(ServerManager.ServerLiveState.UNKNOWN);
    }).map((regionState) -> regionStates.getRegionServerOfRegion(regionState.getRegion())).filter((unknownServer) -> {
      ...
      for (Procedure<MasterProcedureEnv> procedure : currProcedures) {
        ...
        if (procedure instanceof ServerCrashProcedure) {
          if (unknownServer.compareTo(((ServerCrashProcedure) procedure).getServerName()) == 0
            ...
            LOG.info("there is already a SCP of this server {} running, pid {}", unknownServer, procedure.getProcId());
            return false;
          }
        }
      }
  ...
  public void tryScheduleRecoveryForMetaRegionServer() throws IOException {
    List<RegionState> metaRegionStates = regionStates.getTableRegionStates(TableName.META_TABLE_NAME);
    ...
    for (ServerName metaRegionServer : metaRegionServers) {
      if (master.getServerManager().isServerKnownAndOnline(metaRegionServer) != ServerLiveState.LIVE) {
