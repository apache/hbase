...
## Using HBase with SLF4J 2.x

HBase is compiled against SLF4J 1.7 to preserve compatibility with applications that still use it. Applications using SLF4J 2.x can use HBase without rebuilding it: select one `slf4j-api` 2.x version and a matching provider in the application. For Log4j 2, use `log4j-slf4j2-impl` instead of the SLF4J 1.x binding `log4j-slf4j-impl`. Keep only one SLF4J API and provider on the runtime classpath.

The binary distributions also include an optional SLF4J 2.x API and Log4j 2 provider under `lib/slf4j2`. On Unix-like systems, include them in the classpath printed for an application or a MapReduce job with:

```bash
bin/hbase --slf4j2 classpath
bin/hbase --slf4j2 mapredcp
```

These options only change the printed classpath. HBase commands continue to use the default SLF4J 1.x binding. If Hadoop contributes its own logging jars, check the resulting classpath for other SLF4J providers before running the application.
