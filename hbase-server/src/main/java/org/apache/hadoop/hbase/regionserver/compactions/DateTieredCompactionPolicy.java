...
public class DateTieredCompactionPolicy extends SortedCompactionPolicy {
  ...
  public DateTieredCompactionPolicy(Configuration conf, StoreConfigInformation storeConfigInfo)
    ...
    try {
      compactionPolicyPerWindow =
        ReflectionUtils.instantiateWithCustomCtor(comConf.getCompactionPolicyForDateTieredWindow(),
  ...
  protected CompactionRequestImpl createCompactionRequest(ArrayList<HStoreFile> candidateSelection,
    ...
    CompactionRequestImpl result = tryingMajor
  ...
  private DateTieredCompactionRequest generateCompactionRequest(ArrayList<HStoreFile> storeFiles,
    CompactionWindow window, boolean mayUseOffPeak, boolean mayBeStuck, int minThreshold, long now)
    throws IOException {
    ...
    ArrayList<HStoreFile> storeFileSelection = mayBeStuck
      ? storeFiles
  ...
  private List<Long> getCompactBoundariesForMajor(Collection<HStoreFile> filesToCompact, long now) {
    ...
    for (CompactionWindow window = getIncomingWindow(now); window.compareToTimestamp(minTimestamp)
        > 0; window = window.nextEarlierWindow()) {
