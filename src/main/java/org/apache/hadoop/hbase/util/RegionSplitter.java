...
public class RegionSplitter {
  ...
  static void rollingSplit(String tableName, SplitAlgorithm splitAlgo,
    ...
    for (Pair<byte[], byte[]> dr : tmpRegionSet) {
      HServerAddress address = table.getRegionLocation(dr.getSecond()).getServerAddress();
      String rsLocation = address.getHostname() + ":" + address.getPort();
    ...
    try {
      ...
      while (!daughterRegions.isEmpty()) {
        ...
        final TreeMap<HServerAddress, Integer> rsSizes = new TreeMap<HServerAddress, Integer>();
        ...
        for (String rsLoc : serversLeft) {
          ...
          while (!regionList.isEmpty()) {
            ...
            String newRs = regionLoc.getServerAddress().getHostname() + ":" +
                regionLoc.getServerAddress().getPort();
