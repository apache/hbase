...
import org.apache.hadoop.hbase.master.assignment.TestAssignmentManager;
...
//      }
      ...
      for (Result res : tableRegionList) {
        regionsList.add(CatalogAccessor.getRegionInfo(res));
      }
...
//  }
  ...
  private void checkBasicOps(Connection conn, TableName tableName,
    ...
    int i = 0;
    while (r != null) {
      assertEquals(CatalogAccessor.getRegionInfo(r),
        CatalogAccessor.getRegionInfo(expectedList.get(i)));
      r = scanner.next();
      i++;
    }
  ...
  public void mergeRegions(HBaseTestingUtility util, TableName tableName,
    ...
    while (r != null) {
      splitList.add(CatalogAccessor.getRegionInfo(r));
      r = scanner.next();
    }
    scanner.close();
