...
public final class TableName implements Comparable<TableName> {
   ...
   *           characters or underscores: i.e. <code>[\p{IsAlphabetic}\p{Digit}.-:]</code>. The ':'
   *           is used to delimit the namespace from the table name and can be used for nothing
   *           else. Namespace names can only contain 'word' characters
   *           <code>[\p{IsAlphabetic}\p{Digit}]</code> or '_' Qualifier names can only contain
   *           'word' characters <code>[\p{IsAlphabetic}\p{Digit}]</code> or '_', '.' or '-'. The
   *           name may not start with '.' or '-'. Valid fully qualified table names: foo:bar,
   *           namespace=&gt;foo, table=&gt;bar org:foo.bar, namespace=org, table=&gt;foo.bar
   */
   ...
   *           this. The test is buried in the table creation to save on array comparison when we're
   *           creating a standard table object that will be in the cache.
   */
