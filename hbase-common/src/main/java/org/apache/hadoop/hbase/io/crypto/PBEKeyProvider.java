...
public interface PBEKeyProvider extends KeyProvider {
   ...
   * @return PBEKeyData for the system key and is guaranteed to be not {@code null}
   * @throws IOException if an error occurs while retrieving the key
   */
  PBEKeyData getSystemKey(byte[] systemId) throws IOException;
   ...
   * @return PBEKeyData for the system key and is expected to be not {@code null}
   * @throws IOException if an error occurs while retrieving the key
   */
   ...
   * same key provider via the {@link #getSystemKey(byte[])} or {@link #getPBEKey(byte[], String)}
   * methods.
   *
