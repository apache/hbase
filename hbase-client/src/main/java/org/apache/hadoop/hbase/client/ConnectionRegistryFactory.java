...
final class ConnectionRegistryFactory {
   ...
   * {@link ConnectionRegistryCreator} implementation for the given scheme, we will fallback to use
   * the old way to create the {@link ConnectionRegistry}. Notice that, if fallback happens, the
  ...
  static ConnectionRegistry create(URI uri, Configuration conf, User user) throws IOException {
    ...
    ConnectionRegistryCreator creator = CREATORS.get(uri.getScheme());
    if (creator == null) {
