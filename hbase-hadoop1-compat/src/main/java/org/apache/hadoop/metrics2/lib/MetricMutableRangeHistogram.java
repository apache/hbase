...
public abstract class MetricMutableRangeHistogram extends MetricMutableHistogram {
  
  public MetricMutableRangeHistogram(String name, String description) {
  ...
  public void updateSnapshotRangeMetrics(MetricsRecordBuilder metricsRecordBuilder) {
    ...
    for (int i = 0; i < getRange().length; i++) {
      ...
      if (val > 0) {
        metricsRecordBuilder
            .addCounter(name + "_" + getRangeType() + "_" + prior + "-" + getRange()[i], "", val);
      }
      prior = getRange()[i];
    }
    ...
    if (val > 0) {
      metricsRecordBuilder.addCounter(
        name + "_" + getRangeType() + "_" + getRange()[getRange().length - 1] + "-inf", "",
