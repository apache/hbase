...
public abstract class HFileReaderImpl implements HFile.Reader, Configurable {
  ...
  public String toString() {
    return "reader=" + path.toString()
        + (!isFileInfoLoaded() ? ""
            : ", compression=" + trailer.getCompressionCodec().getName() + ", cacheConf="
                + cacheConf + ", firstKey=" + toStringFirstKey() + ", lastKey=" + toStringLastKey())
  ...
  protected static class HFileScannerImpl implements HFileScanner {
     ...
     * that case and load the previous block as appropriate.
    ...
    protected final void checkKeyValueLen() {
      if (checkKeyLen(this.currKeyLen) || checkLen(this.currValueLen)) {
        throw new IllegalStateException(
            "Invalid currKeyLen " + this.currKeyLen + " or currValueLen " + this.currValueLen
                + ". Block offset: " + this.curBlock.getOffset() + ", block length: "
   ...
   * @param pread Use positional read rather than seek+read if true (pread is better for random
   *          reads, seek+read is better scanning).
