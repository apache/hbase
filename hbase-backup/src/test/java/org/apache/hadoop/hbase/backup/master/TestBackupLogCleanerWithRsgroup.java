...
public class TestBackupLogCleanerWithRsgroup extends TestBackupBase {
  ...
  @Test public void testBackupLogCleanerRsgroup() throws Exception {
    // #1 - create full backup for all tables
    LOG.info("create full backup image for all tables");
    List<TableName> tableSetFullList = Lists.newArrayList(RSGROUP_TABLE);
    ...
    try (BackupSystemTable systemTable = new BackupSystemTable(TEST_UTIL.getConnection())) {
      ...
      Table t1 = conn.getTable(RSGROUP_TABLE);
      Put p1;
      ...
      List<TableName> tableSetIncList = Lists.newArrayList(RSGROUP_TABLE);
