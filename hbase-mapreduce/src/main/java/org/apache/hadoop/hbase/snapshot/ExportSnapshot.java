...
public class ExportSnapshot extends AbstractHBaseTool implements Tool {
  ...
  public enum Counter {
    MISSING_FILES, FILES_COPIED, FILES_SKIPPED, COPY_FAILED, BYTES_EXPECTED, BYTES_SKIPPED,
    BYTES_COPIED
  }
  ...
  private static class ExportMapper
    ...
    private void copyData(final Context context, final Path inputPath, final InputStream in,
      ...
      try {
        ...
        LOG.info(
          "size=" + totalBytesWritten + " (" + StringUtils.humanReadableInt(totalBytesWritten) + ")"
  ...
  public int doWork() throws IOException {
    ...
    if (outputRoot == null) {
      System.err.println(
        "Destination file-system (--" + Options.COPY_TO.getLongOpt() + ") not provided.");
      LOG.error("Use -h or --help for usage instructions.");
      return 0;
    }
