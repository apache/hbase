...
public class WALUtil {
  ...
  public static WALKeyImpl writeRegionEventMarker(WAL wal,
    ...
    throws IOException {
    WALKeyImpl walKey =
        writeMarker(wal, replicationScope, hri, WALEdit.createRegionEventWALEdit(hri, r), mvcc);
    if (LOG.isTraceEnabled()) {
  ...
  public static WALKeyImpl writeBulkLoadMarkerAndSync(final WAL wal,
    ...
    WALKeyImpl walKey = writeMarker(wal, replicationScope, hri,
      WALEdit.createBulkLoadEvent(hri, desc), mvcc);
    if (LOG.isTraceEnabled()) {
  ...
  private static WALKeyImpl writeMarker(final WAL wal, NavigableMap<byte[],
      Integer> replicationScope, RegionInfo hri, WALEdit edit, MultiVersionConcurrencyControl mvcc)
    throws IOException {
  ...
  private static WALKeyImpl doFullMarkerAppendTransaction(final WAL wal,
    ...
    final MultiVersionConcurrencyControl mvcc, final boolean sync)
    throws IOException {
