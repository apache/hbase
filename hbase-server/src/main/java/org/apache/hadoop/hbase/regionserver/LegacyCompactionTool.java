...
/*
 * The LegacyCompactionTool allows to execute a compaction specifying a:
 * <ul>
...
public class LegacyCompactionTool extends Configured implements Tool {
  private static final Log LOG = LogFactory.getLog(LegacyCompactionTool.class);
  ...
  private static class CompactionWorker {
    ...
    public void compact(final Path path, final boolean compactOnce, final boolean major) throws IOException {
      if (isFamilyDir(fs, path)) {
        ...
        compactStoreFiles(htd, rs.getRegionInfo(), path.getName(), compactOnce, major);
    ...
    private void compactRegion(final Path tableDir, final HTableDescriptor htd,
        ...
        throws IOException {
      final RegionStorage<? extends StorageIdentifier> rs =
          RegionStorage.open(conf, new LegacyPathIdentifier(regionDir), false);
      for (String familyName: rs.getFamilies()) {
        compactStoreFiles(htd, rs.getRegionInfo(), familyName, compactOnce, major);
      }
    }
    ...
    private static HStore getStore(final Configuration conf, final FileSystem fs,
        final HTableDescriptor htd, final HRegionInfo hri, final String familyName)
        throws IOException {
      RegionStorage rs = RegionStorage.open(conf, hri, false);
  ...
  private static boolean isRegionDir(final FileSystem fs, final Path path) throws IOException {
    return fs.exists(path) && isTableDir(fs, path.getParent());
  }

  private static boolean isTableDir(final FileSystem fs, final Path path) throws IOException {
    try {
      return fs.exists(path) && LegacyTableDescriptor.getTableInfoPath(fs, path) != null;
    } catch (TableInfoMissingException e) {
      LOG.info("'" + path + "' not a table directory. " + e);
    }
    return false;
  }

  private static boolean isFamilyDir(final FileSystem fs, final Path path) throws IOException {
    return fs.exists(path) && isRegionDir(fs, path.getParent());
  }
  ...
  private int doMapReduce(final FileSystem fs, final Set<Path> toCompactDirs,
    ...
    job.setJobName("LegacyCompactionTool");
    job.setJarByClass(LegacyCompactionTool.class);
  ...
  public static void main(String[] args) throws Exception {
    System.exit(ToolRunner.run(HBaseConfiguration.create(), new LegacyCompactionTool(), args));
  }
}
