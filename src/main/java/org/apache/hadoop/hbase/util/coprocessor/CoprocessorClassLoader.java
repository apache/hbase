 ...
 * will be copied to local at first under some folder under
 * ${hbase.local.dir}/jars/tmp/. The local copy will be removed automatically
 * when the HBase server instance is stopped.
 * <p>
 ...
 * before delegating to the parent. However, like in class loading, some
 * resources need to be handled differently. For all of the Hadoop default
 * configurations (e.g. hbase-default.xml) we will check the parent ClassLoader
 * first to prevent issues such as failing the HBase default configuration
 * version check.
 */
public class CoprocessorClassLoader extends URLClassLoader {
  private static final Log LOG = LogFactory
      .getLog(CoprocessorClassLoader.class);

  // A temporary place ${hbase.local.dir}/jars/tmp/ to store the local
  // copy of the jar file and the libraries contained in the jar.
  private static final String TMP_JARS_DIR = File.separator + "jars"
      + File.separator + "tmp" + File.separator;

  /**
   * External class loaders cache keyed by external jar path. ClassLoader
   * instance is stored as a weak-reference to allow GC'ing when it is not used
   * (@see HBASE-7205)
   */
  ...
  private static final Pattern libJarPattern = Pattern
      .compile("[/]?lib/([^/]+\\.jar)");

  /**
   * A locker used to synchronize class loader initialization per coprocessor
   * jar file
   */
  private static final KeyLocker<String> locker = new KeyLocker<String>();

  /**
   * A set used to synchronized parent path clean up. Generally, there should be
   * only one parent path, but using a set so that we can support more.
   */
  ...
  private void init(Path path, String pathPrefix, Configuration conf)
      throws IOException {
    // Copy the jar to the local filesystem
    String parentDirStr = conf.get(LOCAL_DIR_KEY, DEFAULT_LOCAL_DIR)
        + TMP_JARS_DIR;
    synchronized (parentDirLockSet) {
      if (!parentDirLockSet.contains(parentDirStr)) {
        ...
        if (!fs.mkdirs(parentDir) && !fs.getFileStatus(parentDir).isDir()) {
          throw new RuntimeException("Failed to create local dir "
              + parentDirStr + ", CoprocessorClassLoader failed to init");
        }
      }
    }
  ...
  /**
   * Get a CoprocessorClassLoader for a coprocessor jar path from cache. If not
   * in cache, create one.
   *
   * @param path
  ...
  public static CoprocessorClassLoader getClassLoader(final Path path,
    ...
    try {
      ...
      cl = AccessController
          .doPrivileged(new PrivilegedAction<CoprocessorClassLoader>() {
            @Override
      ...
      // Cache class loader as a weak value, will be GC'ed when no reference
      // left
   ...
   * @return true if the class should *not* be loaded by this ClassLoader; false
   *         otherwise.
   */
