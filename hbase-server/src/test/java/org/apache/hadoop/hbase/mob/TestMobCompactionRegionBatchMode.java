...
/**
  ...
  * 12 Runs scanner and checks all 10000 rows.
 */
...
public class TestMobCompactionRegionBatchMode {
  ...
  private MobFileCleanerChore cleanerChore;
  ...
  public void testMobFileCompactionBatchMode() throws InterruptedException, IOException {
    ...
    LOG.info("MOB compaction chore finished OK");
  ...
  private long scanTable() {
    try {
      ...
      e.printStackTrace();
      LOG.error("MOB file compaction chore test FAILED");
      if (HTU != null) {
        assertTrue(false);
      } else {
        System.exit(-1);
