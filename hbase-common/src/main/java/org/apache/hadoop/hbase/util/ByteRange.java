 ...
 * <p>
 * {@code ByteRange} maintains an underlying byte[] and a viewport into that byte[] as a range of
 * bytes. The {@code ByteRange} is a mutable, reusable object, so the underlying byte[] can be
 * modified after instantiation. This is done using the {@link #set(byte[])} and {@link #unset()}
 * methods. Direct access to the byte[] is also available via {@link #getBytes()}. The viewport is
 * defined by an {@code offset} into the byte[] and a {@code length}. The range of bytes is
 * 0-indexed, and is accessed by index via the {@link #get(int)} and {@link #put(int, byte)}
 * methods.
 * </p>
 * <p>
 ...
 * <li>Implements {@code equals(Object)}, {@code #hashCode()}, and {@code #compareTo(ByteRange)} so
 ...
 * </ul>
 * <p>
 * Mutable, and always evaluates {@code #equals(Object)}, {@code #hashCode()}, and
 * {@code #compareTo(ByteRange)} based on the current contents.
 * </p>
 * <p>
...
public interface ByteRange extends Comparable<ByteRange> {
  ...
  /**
   * Reuse this {@code ByteRange} over a new byte[]. A null {@code bytes} IS supported, in which
   * case this method will behave equivalently to {@link #unset()}, regardless of the values of
  ...
  /**
   * Fill {@code dst} with bytes from the range, starting from {@code index}. {@code length} bytes
   * are copied into {@code dst}, starting at {@code offset}.
   * @param index  zero-based index into this range.
   * @param dst    the destination of the copy.
  ...
  /**
   * Store {@code length} bytes from {@code val} into this range, starting at {@code index}. Bytes
   * from {@code val} are copied starting at {@code offset} into the range.
  ...
  /**
   * Create a new {@code ByteRange} with new backing byte[] containing a copy of the content from
   * {@code this} range's window.
   * @return Deep copy
   */
  ...
  /**
   * Wrapper for System.arraycopy. Copy the contents of this range into the provided array.
   * @param innerOffset       Start copying from this index in this source ByteRange. First byte
   *                          copied is bytes[offset + innerOffset]
  ...
  /**
   * Create a new {@code ByteRange} that points at this range's byte[]. Modifying the shallowCopy
   * will modify the bytes in this range's array. Pass over the hash code if it is already cached.
   * @return new {@code ByteRange} object referencing this range's byte[].
   */
  public ByteRange shallowCopy();

  /**
   * Create a new {@code ByteRange} that points at this range's byte[]. The new range can have
   * different values for offset and length, but modifying the shallowCopy will modify the bytes in
   * this range's array. Pass over the hash code if it is already cached.
