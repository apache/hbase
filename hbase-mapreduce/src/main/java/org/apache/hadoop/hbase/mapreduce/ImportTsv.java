...
public class ImportTsv extends Configured implements Tool {
  ...
  public static class TsvParser {
    ...
    public TsvParser(String columnsSpecification, String separatorStr) {
      ...
      ArrayList<String> columnStrings =
          Lists.newArrayList(Splitter.on(',').trimResults().split(columnsSpecification));
    ...
    class ParsedLine {
      ...
      public int getColumnOffset(int idx) {
        if (idx > 0) return tabOffsets.get(idx - 1) + 1;
        else return 0;
      }
  ...
  private static void createTable(Admin admin, TableName tableName, String[] columns)
    ...
    LOG.warn(
      format("Creating table '%s' with '%s' columns and default descriptors.", tableName, cfSet));
  ...
  private static void usage(final String errorMsg) {
    ...
    String usage = "Usage: " + NAME + " -D" + COLUMNS_CONF_KEY + "=a,b,c <tablename> <inputdir>\n"
        ...
        + "Note: if you use this option, then '" + TIMESTAMP_CONF_KEY
        + "' option will be ignored.\n" + "\n" + "Other special columns that can be specified are "
        + TsvParser.CELL_TTL_COLUMN_SPEC + " and " + TsvParser.CELL_VISIBILITY_COLUMN_SPEC + ".\n"
        ...
        + DRY_RUN_CONF_KEY + "=true - Dry run mode. Data is not actually populated into"
        + " table. If table does not exist, it is created but deleted in the end.\n" + "  -D"
        + SKIP_LINES_CONF_KEY + "=false - fail if encountering an invalid line\n" + "  -D"
        + LOG_BAD_LINES_CONF_KEY + "=true - logs invalid lines to stderr\n" + "  -D"
        + SKIP_EMPTY_COLUMNS + "=false - If true then skip empty columns in bulk import\n" + "  '-D"
        + SEPARATOR_CONF_KEY + "=|' - eg separate on pipes instead of tabs\n" + "  -D"
        + TIMESTAMP_CONF_KEY + "=currentTimeAsLong - use the specified timestamp for the import\n"
  ...
  public int run(String[] args) throws Exception {
    ...
    if (null == getConf().get(MAPPER_CONF_KEY)) {
      ...
      if (columns.length - (rowkeysFound + tskeysFound + attrKeysFound) < 1) {
        usage(
          "One or more columns in addition to the row key and timestamp(optional) are required");
        return -1;
      }
    }
