...
public class TestCheckAndMutateWithByteBuff {
  ...
  private static Table scanTable;
  ...
  public static void setupBeforeClass() throws Exception {
    ...
    conf.setInt(ByteBuffAllocator.BUFFER_SIZE_KEY, 1000);
  ...
  public void testCheckAndMutateWithByteBuff() throws Exception {
    ...
    scanTable = createTable(TableName.valueOf(name.getMethodName() + "_2"));
    Put put1 = new Put(Bytes.toBytes("row2"));
    put1.addColumn(CF, Bytes.toBytes("cq2"), Bytes.toBytes("value2"));
    scanTable.put(put1);
    admin.flush(scanTable.getName());
  ...
  /**
   * An override of HRegion to allow sleep and scan other table blocks after get(),
   * waiting for the release and reuse of DBB.
   */
  public static class TestCheckAndMutateRegion extends HRegion {
    ...
    public List<Cell> get(Get get, boolean withCoprocessor) throws IOException {
      ...
      sleep(600);
      try (ResultScanner rs = scanTable.getScanner(new Scan())) {
        rs.next();
      }
      return cells;
    }
