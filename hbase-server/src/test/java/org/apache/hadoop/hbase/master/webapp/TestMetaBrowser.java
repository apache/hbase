...
public class TestMetaBrowser {
  ...
  public void noFilters() {
    ...
    CompletableFuture.allOf(createTable(a),
  ...
  public void limit() {
    ...
    assertThat(rows,
      contains(hasProperty("row", bytesAsStringBinary(startsWith("hbase:namespace,,"))),
  ...
  public void paginateWithReplicas() {
    ...
    final HttpServletRequest request3 = new MockRequestBuilder().setLimit(2)
    ...
    assertThat(rows3,
      contains(hasProperty("row", bytesAsStringBinary(startsWith(b + ",80000000")))));
  }
  ...
  public void paginateWithTableFilter() {
    ...
    CompletableFuture.allOf(createTable(a),
      createNamespace(namespaceName).thenCompose(_void -> createTable(b, 5))).join();
