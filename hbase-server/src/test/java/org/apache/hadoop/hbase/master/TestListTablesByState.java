...
public class TestListTablesByState {
  ...
  public void testListTableDescriptorByState() throws Exception {
    ADMIN.createTable(TABLE_DESC);
    ADMIN.disableTable(TABLE);
    assertEquals(ADMIN.listTableDescriptorsByState(false).get(0).getTableName(), TABLE);
    ADMIN.enableTable(TABLE);
    assertEquals(ADMIN.listTableDescriptorsByState(true).get(0).getTableName(), TABLE);
  }
