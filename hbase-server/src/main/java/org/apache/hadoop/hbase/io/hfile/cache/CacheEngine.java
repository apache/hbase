 ...
 *
 * <p>A {@code CacheEngine} represents the storage layer only. It is responsible for storing,
 * retrieving, invalidating, and reporting statistics for cached blocks. It does not perform
 * tier orchestration, admission control, placement decisions, or promotion/demotion across
 * cache levels.</p>
 *
 * <p>This interface is intentionally aligned with the storage-oriented subset of the current
...
public interface CacheEngine {
   ...
   *
   * <p>The name is intended for logging, metrics, diagnostics, and configuration reporting.
   ...
   *
   * <p>This identifies the concrete backend family, such as LRU, BUCKET, or CARROT. The type
   ...
   *
   * <p>This is primarily useful for implementations such as BucketCache that may buffer writes
   * asynchronously.</p>
   *
   * @param cacheKey block cache key
   * @param buf block contents
   ...
   *
   * <p>This is useful for targeted invalidation during file lifecycle events where only a subset
   ...
   *
   * <p>This is a new API intended to support region-scoped invalidation in a storage-oriented
   ...
   *
   * <p>This method is optional because not all engines expose a meaningful fit check.</p>
   *
   ...
   *
   * <p>This method is optional because not all engines can answer it efficiently.</p>
   *
   ...
   *
   * <p>This method is optional because not all engines expose per-block size cheaply.</p>
   *
   * @param key block cache key
   ...
   *
   * <p>Some cache backends may perform initialization asynchronously. Engines that do not
   * require this may simply return {@code true} immediately.</p>
   *
