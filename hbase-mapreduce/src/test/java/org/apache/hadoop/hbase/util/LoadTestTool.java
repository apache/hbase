...
public class LoadTestTool extends AbstractHBaseTool {
  ...
  protected String[] splitColonSeparated(String option, int minNumCols, int maxNumCols) {
    ...
    if (cols.length < minNumCols || cols.length > maxNumCols) {
      throw new IllegalArgumentException("Expected at least " + minNumCols
          + " columns but no more than " + maxNumCols + " in the colon-separated value '" + optVal
    ...
    try (Connection conn = ConnectionFactory.createConnection(conf);
      ...
      for (byte[] cf : columnFamilies) {
        ...
        ColumnFamilyDescriptorBuilder columnDescBuilder =
            isNewCf ? ColumnFamilyDescriptorBuilder.newBuilder(cf)
