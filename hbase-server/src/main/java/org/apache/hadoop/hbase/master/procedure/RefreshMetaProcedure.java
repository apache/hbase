...
public class RefreshMetaProcedure extends AbstractStateMachineTableProcedure<RefreshMetaState> {
  ...
  private Flow executeUpdate(MasterProcedureEnv env) throws IOException {
    ...
    try {
      ...
      compareAndUpdateRegions(currentRegions, latestRegions, env.getMasterServices().getConnection(), env);
   ...
   * and applies necessary updates (additions, deletions, or modifications) to the meta table.
   *
   * @param current Current regions from hbase:meta
   * @param latest Latest regions from backing storage
   ...
   */
  private void compareAndUpdateRegions(List<RegionInfo> current, List<RegionInfo> latest,
    Connection connection, MasterProcedureEnv env) throws IOException {

    List<Mutation> mutations = new ArrayList<>();
    Map<String, RegionInfo> currentMap = current.stream()
  ...
  private List<RegionInfo> scanBackingStorage(Connection connection) throws IOException {
    ...
    List<FileStatus> namespaceDirs = Arrays.stream(fs.listStatus(dataDir, this::isRelevantDirectory)).toList();
    LOG.debug("Found {} namespace directories in data dir", namespaceDirs);

    for (FileStatus nsDir : namespaceDirs) {
      try {
        List<RegionInfo> namespaceRegions = scanNamespaceDirectory(fs, nsDir.getPath());
  ...
  private List<RegionInfo> scanNamespaceDirectory(FileSystem fs, Path namespacePath) throws IOException {
    List<RegionInfo> regions = new ArrayList<>();
    String namespaceName = namespacePath.getName();

    List<FileStatus> allTableDirs = Arrays.stream(fs.listStatus(namespacePath, this::isRelevantDirectory)).toList();
    LOG.debug("Scanning namespace {} with {} tables)", namespaceName, allTableDirs.size());

    for (FileStatus tableDir : allTableDirs) {
      if (isValidTableDirectory(tableDir, namespaceName)) {
        try {
          // Get all region directories in this table
          FileStatus[] regionDirs = fs.listStatus(tableDir.getPath(), this::isRelevantDirectory);
          List<RegionInfo> tableRegions = scanRegionDirectories(fs, regionDirs);
          regions.addAll(tableRegions);

          LOG.debug("Found {} regions in table {}", tableRegions.size(), tableDir.getPath().getName());
        } catch (IOException e) {
          LOG.warn("Failed to scan table directory: {}", tableDir.getPath(), e);
        }
      }
    }
    return regions;
  ...
  private List<RegionInfo> scanRegionDirectories(FileSystem fs, FileStatus[] regionDirs) throws IOException {
    List<RegionInfo> regions = new ArrayList<>();

    for (FileStatus regionDir : regionDirs) {
      String encodedRegionName = regionDir.getPath().getName();
      if (isValidRegionDirectory(regionDir)) {
        try {
          List<FileStatus> regionFiles = Arrays.stream(fs.listStatus(regionDir.getPath(),
            path -> path.getName().equals(HRegionFileSystem.REGION_INFO_FILE))).toList();

          if (!regionFiles.isEmpty()) {
            RegionInfo ri = readRegionInfo(fs, regionFiles.get(0).getPath());
            if (ri != null && isValidRegionInfo(ri, encodedRegionName)) {
              regions.add(ri);
              LOG.debug("Found region: {} -> {}", encodedRegionName, ri.getRegionNameAsString());
            } else {
              LOG.warn("Invalid RegionInfo in file: {}", regionFiles.get(0).getPath());
            }
          } else {
            LOG.debug("No .regioninfo file found in region directory: {}", regionDir.getPath());
          }
        } catch (Exception e) {
  ...
  private boolean isValidRegionDirectory(FileStatus fileStatus) {
    String regionDirName = fileStatus.getPath().getName();
    if (!fileStatus.isDirectory() || !isRelevantDirectory(fileStatus.getPath()) || !RegionInfo.isEncodedRegionName(Bytes.toBytes(regionDirName))) {
      LOG.debug("Directory {} is not a valid region directory", regionDirName);
      return false;
    }
    return true;
  }

  private boolean isValidTableDirectory(FileStatus fileStatus, String namespaceName) {
    String tableDirName = fileStatus.getPath().getName();
    if (!fileStatus.isDirectory() || !isRelevantDirectory(fileStatus.getPath())) {
      return false;
    }
    if (NamespaceDescriptor.SYSTEM_NAMESPACE_NAME_STR.equals(namespaceName)
      && (tableDirName.startsWith(TableName.META_TABLE_NAME.getNameAsString()))) {
      LOG.debug("Excluding meta from scan: {}", tableDirName);
      return false;
    }
    return true;
  }
  ...
  private boolean isValidRegionInfo(RegionInfo regionInfo, String expectedEncodedName) {
    String actualEncodedName = regionInfo.getEncodedName();
    if (!expectedEncodedName.equals(actualEncodedName)) {
      LOG.warn("RegionInfo encoded name mismatch: directory={}, regioninfo={}",
        expectedEncodedName, actualEncodedName);
      return false;
    }
    return true;
  }
