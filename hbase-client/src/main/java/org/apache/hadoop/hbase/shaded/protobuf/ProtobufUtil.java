...
public final class ProtobufUtil {
   ...
   *          is set to true, it will return all the tags in the response. These tags may contain
   *          some sensitive data like acl permissions, etc. Only the tools like Export, Import
   *          which needs to take backup needs to set it to true so that cell tags are persisted in
   *          backup. Refer to HBASE-25246 for more context.
   * @return the converted protocol buffer Result
   */
   ...
   *          to true, it will decode all the tags from the response. These tags may contain some
   *          sensitive data like acl permissions, etc. Only the tools like Export, Import which
   *          needs to take backup needs to set it to true so that cell tags are persisted in
   *          backup. Refer to HBASE-25246 for more context.
   * @return the converted client Result
   */
   ...
   */
  public static List<org.apache.hadoop.hbase.client.RegionInfo>
      getOnlineRegions(final RpcController controller, final AdminService.BlockingInterface admin)
          throws IOException {
  ...
  public static RegionEventDescriptor toRegionEventDescriptor(EventType eventType,
    ...
    RegionEventDescriptor.Builder desc = RegionEventDescriptor.newBuilder().setEventType(eventType)
        ...
        .setLogSequenceNumber(seqId).setServer(toServerName(server));
  ...
  public static String getShortTextFormat(Message m) {
    ...
    if (m instanceof ScanRequest) {
      ...
      int actionsCount = r.getRegionActionList().stream()
      ...
      String row = actions.getActionCount() <= 0 ? ""
          : getStringForByteString(
            actions.getAction(0).hasGet() ? actions.getAction(0).getGet().getRow()
  ...
  public static TimeRange toTimeRange(HBaseProtos.TimeRange timeRange) {
    return timeRange == null ? TimeRange.allTime()
  ...
  /**
   * Convert from {@link org.apache.hadoop.hbase.protobuf.generated.HBaseProtos.SnapshotDescription}
   * to {@link SnapshotDescription}
