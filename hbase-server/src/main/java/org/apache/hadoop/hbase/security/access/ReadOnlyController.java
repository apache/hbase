...
public class ReadOnlyController implements MasterCoprocessor, RegionCoprocessor,
  MasterObserver, RegionObserver, RegionServerCoprocessor, RegionServerObserver,
  ...
  private void internalReadOnlyGuard() throws IOException{
    if (conf.getBoolean(HConstants.HBASE_GLOBAL_READONLY_ENABLED_KEY,
  ...
  @Override public void preDeleteTableAction(ObserverContext<MasterCoprocessorEnvironment> ctx,
  ...
  @Override public void preTruncateTableAction(ObserverContext<MasterCoprocessorEnvironment> ctx,
    TableName tableName) throws IOException {
    internalReadOnlyGuard();
