...
/**
 * The cluster status checker for {@StochasticLoadBalancer}, if the skew table counts up to a
 * configured ratio, the simplified balancer will be executed, it is aimed to balance the regions
 * as soon as possible.
 *
 * For example, when there are large-scaled restart of RSes, or expansion for groups or cluster,
 * we hope the balancer can execute as soon as possible, but the StochasticLoadBalancer may
 * need a lot of time to compute costs.
 */
...
class OverallSkewChecker implements StochasticLoadBalancer.ClusterStatusBalanceChecker{
  ...
  @Override
  public boolean needsSimplifiedBalance(
    ...
    if (isClusterSkew(loadOfAllTable)) {
      LOG.debug("Balancer checker found cluster is skew, will perform force balance, "
          + "tables count is {}", loadOfAllTable.size());
      return true;
    }
  ...
  public boolean isClusterSkew(Map<TableName, Map<ServerName, List<RegionInfo>>> loadOfAllTable) {
    ...
    for (Map.Entry<TableName, Map<ServerName, List<RegionInfo>>> entry : loadOfAllTable
      ...
      if (isSkew(entry.getValue())) {
        ...
        if(LOG.isDebugEnabled()) {
          StringBuilder sb = new StringBuilder("Table: " + entry.getKey().getNameAsString() +
            " regions distribution is: ");
          for(Map.Entry<ServerName, List<RegionInfo>> regionDistribution : entry.getValue().entrySet()) {
            sb.append(regionDistribution.getKey().getServerName()).append(" count=")
              .append(regionDistribution.getValue().size())
              .append(":");
    ...
    return skewCount > 0 && (double) skewCount / loadOfAllTable.size() > conf
      .getDouble(BALANCER_OVERALL_SKEW_PERCENT_KEY, 0.5);
  }
