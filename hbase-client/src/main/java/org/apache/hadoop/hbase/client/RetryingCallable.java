...
/**
 * A Callable&lt;T&gt; that will be retried. If {@link #call(int)} invocation throws exceptions, we
 * will call {@link #throwable(Throwable, boolean)} with whatever the exception was.
 * @param <T> result class from executing <tt>this</tt>
 */
@InterfaceAudience.Private
public interface RetryingCallable<T> {
  /**
   * Prepare by setting up any connections to servers, etc., ahead of call invocation. TODO: We call
   * prepare before EVERY call. Seems wrong. FIX!!!!
   ...
   * @param retrying True if we are in retrying mode (we are not in retrying mode when max retries
   *          == 1; we ARE in retrying mode if retries &gt; 1 even when we are the last attempt)
   */
