...
public class ArrayBackedTag implements Tag {
   ...
   * then the actual tag. So every time the length part is parsed we need to add + 1 byte to it to
  ...
  /**
   * Format for a tag : {@code <length of tag - 2 bytes><type code - 1 byte><tag>} tag length is
  ...
  /**
   * Creates a Tag from the specified byte array and offset. Presumes <code>bytes</code> content
   * starting at <code>offset</code> is formatted as a Tag blob. The bytes to include the tag type,
