...
@InterfaceAudience.Private public class OnlineMergeTool extends Configured implements Tool {
  ...
  // Name of Region Create Timestamp
  private Long regionCretaeTime = 0L;
  ...
  private boolean doCommandLine(final String[] args) {
    ...
    try {
      for (int i = 0; i < args.length; i++) {
        ...
        final String maxRegionCreateTimeKey = "--regionCretaeTime=";
        if (cmd.startsWith(maxRegionCreateTimeKey)) {
          regionCretaeTime = Long.parseLong(cmd.substring(maxRegionCreateTimeKey.length()));
          continue;
        }
  ...
  private List<Pair<byte[], byte[]>> executionPlan() throws IOException, ParseException {
    ...
    if (hris.size() < 2) {
      throw new IOException(
        "The number of regions must be greater than 2" + ". The number of current regions is "
          + hris.size());
    }
    ...
    for (int i = 0; i < hris.size() - 1; i += 2) {
      ...
      if (0L != regionCretaeTime) {
        if (regionInfo1.getRegionId() > regionCretaeTime
          || regionInfo2.getRegionId() > regionCretaeTime) {
          ...
          mesg.append(" > maxRegionCreateTime=").append(regionCretaeTime);
  ...
  private void printCompletedMesg() {
    ...
    mesg.append(" RegionCretaeTimestamp=").append(regionCretaeTime);
  ...
  private void runCompaction(String tableName) throws IOException, InterruptedException {
    while (true) {
      ...
      if (!compactionState.startsWith(COMPACTIONATTRIBUTE)) {
  ...
  private void printUsage(final String errorMsg) {
    ...
    System.err.println("Usage: bin/hbase onlinemerge [--tableName=] "
      ...
      + "[--regionCretaeTime=] [--numMaxMergePlans=] "
    ...
    System.err.println("--startRegion           full region name of start region");
    System.err.println("--stopRegion            full region name of stop region");
    System.err.println("--maxRegionSize         max size of target region(Unit GB)");
    System.err.println("--regionCretaeTime      value is Unix timestamp");
    System.err.println("--numMaxMergePlans      the region count of per merge plan");
    System.err.println("--targetRegionCount     target Region Count");
    System.err.println("--configMergePauseTime  merge pause time in milliseconds");
    System.err.println("--printExecutionPlan    value can be true or false "
      + "default is true, print execution plans, false is execution merge\n");
    ...
    System.err.println("bin/hbase onlinemerge --tableName=test:test1 "
      ...
      + "--maxRegionSize=0 --regionCretaeTime=1573044786980 "
