...
public class TestVerifyCellsReplicationEndpoint {
  ...
  public static final class EndpointForTest extends VerifyWALEntriesReplicationEndpoint {
    ...
    public boolean replicate(ReplicateContext replicateContext) {
      ...
      for (WAL.Entry entry: replicateContext.getEntries()) {
        CELLS.addAll(entry.getEdit().getCells());
      }
  ...
  public static void setUp() throws Exception {
    ...
    peerConfig.setClusterKey("zk1:8888:/hbase");
    peerConfig.setReplicationEndpointImpl(EndpointForTest.class.getName());
    UTIL.createTable(TABLE_NAME, CF);
    try (ReplicationAdmin replAdmin = new ReplicationAdmin(UTIL.getConfiguration())) {
      replAdmin.addPeer(PEER_ID, peerConfig);
    }
  }
