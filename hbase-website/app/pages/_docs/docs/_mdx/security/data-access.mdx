...
2. Enable SASL and Kerberos authentication for RPC and ZooKeeper, as described in [security.prerequisites](#security.prerequisites) and [ZooKeeper](/docs/zookeeper).
...
2.  Install and configure the AccessController coprocessor, by setting the following properties in _hbase-site.xml_. These properties take a list of classes.
    ...
    <Callout type="info">
      ...
      delegate access control on its system tables to the AccessController. For an example of using
      both together, see [Security Configuration Example](#security.example.config).
    </Callout>
...
#### ACL Granularity and Evaluation Order [!toc]
...
3.  Revoking Access Control From a Namespace, Table, Column Family, or Cell
    ...
    <Callout type="info">
      HBase Shell support for granting and revoking access is for testing and verification support,
      and should not be employed for production use because it won’t apply the permissions to cells
      that don’t exist yet. The correct way to apply cell-level permissions is to do so in the
      application code when storing the values.
    </Callout>

#### Revoking Access To a Table example:
...
4.  Apply a Label or Expression to a Cell
    ...
    <Callout type="info">
      HBase Shell support for applying labels or permissions to cells is for testing and
      verification support, and should not be employed for production use because it won't apply the
      labels to cells that don't exist yet. The correct way to apply cell level labels is to do so
      in the application code when storing the values.
    </Callout>

    #### Java API example: [!toc]
...
4.  Configure WAL encryption.
    ...
    <Callout type="info">
      ...
      hbase.regionserver.hlog.writer.impl configurations are removed, you do not need to specify
      them any more. Just set hbase.regionserver.wal.encryption to true is enough to enable WAL
      encryption.
    </Callout>
...
6.  Configure permissions on the _hbase-site.xml_ file.
