...
public class TestImmutableHColumnDescriptor {
  ...
  private static final List<Consumer<ImmutableHColumnDescriptor>> TEST_FUNCTION = Arrays.asList(
    ...
    hcd -> hcd.removeConfiguration("xxx"), hcd -> hcd.setBlockCacheEnabled(false),
    hcd -> hcd.setBlocksize(10), hcd -> hcd.setBloomFilterType(BloomType.NONE),
    hcd -> hcd.setCacheBloomsOnWrite(false), hcd -> hcd.setCacheDataOnWrite(true),
    hcd -> hcd.setCacheIndexesOnWrite(true),
    hcd -> hcd.setCompactionCompressionType(Compression.Algorithm.LZO),
    hcd -> hcd.setCompressTags(true), hcd -> hcd.setCompressionType(Compression.Algorithm.LZO),
    hcd -> hcd.setDFSReplication((short) 10),
    hcd -> hcd.setDataBlockEncoding(DataBlockEncoding.NONE),
    hcd -> hcd.setEncryptionKey(Bytes.toBytes("xxx")), hcd -> hcd.setEncryptionType("xxx"),
    hcd -> hcd.setEvictBlocksOnClose(true), hcd -> hcd.setInMemory(true),
    hcd -> hcd.setInMemoryCompaction(MemoryCompactionPolicy.NONE),
    hcd -> hcd.setKeepDeletedCells(KeepDeletedCells.FALSE), hcd -> hcd.setMaxVersions(1000),
    hcd -> hcd.setMinVersions(10),
    hcd -> hcd.setMobCompactPartitionPolicy(MobCompactPartitionPolicy.DAILY),
    hcd -> hcd.setMobEnabled(true), hcd -> hcd.setMobThreshold(10),
    hcd -> hcd.setPrefetchBlocksOnOpen(true), hcd -> hcd.setScope(0),
    hcd -> hcd.setStoragePolicy("aaa"), hcd -> hcd.setTimeToLive(100),
