...
public class TestRefreshMetaProcedureIntegration {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(TestRefreshMetaProcedureIntegration.class);
  ...
  /**
   * A test for RefreshMetaProcedure to test the workflow:
   * 1. Write data and create regions in active mode
   * 2. Switch to read-only mode
   * 3. Use refresh_meta to sync meta table with storage and switch back to active mode
   */
  @Test
  public void testRestoreMissingRegionInMeta() throws Exception {

    // Phase 1: Active cluster - create tables and regions
    ...
    // Dynamically enable read-only mode
    setReadOnlyMode(true);

    // Verify read-only mode is active by trying a write operation
    ...
    try {
      ...
      if (e.getMessage().contains("Operation not allowed in Read-Only Mode")) {
        writeBlocked = true;
        LOG.info("Write operation correctly blocked in read-only mode");
      }
    }
    assertTrue("Write operations should be blocked in read-only mode", writeBlocked);

    // Phase 3: Use refresh_meta to sync meta table with storage
    ...
    TEST_UTIL.waitFor(3000, () -> {
      try {
        ...
        LOG.warn("Error checking procedure status", e);
        return false;
      }
    });
    ...
    // Phase 4: Switch back to active mode
    ...
    testPut.addColumn(Bytes.toBytes("cf1"), Bytes.toBytes("qual"), Bytes.toBytes("active_mode_again"));
    activeTable.put(testPut);
    activeTable.close();
  }

  private void setReadOnlyMode(boolean isReadOnly) {
    TEST_UTIL.getConfiguration().setBoolean(HConstants.HBASE_GLOBAL_READONLY_ENABLED_KEY, isReadOnly);
