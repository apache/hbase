...
description: "Quick start to get you up and run a single-node, standalone instance of HBase."
---
...
## JDK Version Requirements [#getting-started-jdk-version-requirements]
...
_Example extract from *hbase-env.sh* where *JAVA_HOME* is set_
...
The _bin/start-hbase.sh_ script is provided as a convenient way to start HBase. Issue the command, and if all goes well, a message is logged to standard output showing that HBase started successfully. You can use the `jps` command to verify that you have one running process called `HMaster`. In standalone mode HBase runs all daemons within this single JVM, i.e. the HMaster, a single HRegionServer, and the ZooKeeper daemon. Go to _[http://localhost:16010](http://localhost:16010)_ to view the HBase Web UI.
...
#### Stop HBase [#getting-started-pseudo-distributed-for-local-testing-stop-hbase]

You can stop HBase the same way as in the [quickstart](/docs/getting-started#quick-start---standalone-hbase) procedure, using the _bin/stop-hbase.sh_ command.
...
Since `node-b` will run a backup Master, repeat the procedure above, substituting `node-b` everywhere you see `node-a`. Be sure not to overwrite your existing _.ssh/authorized_keys_ files, but concatenate the new key onto the existing file using the `>>` operator rather than the `>` operator.
