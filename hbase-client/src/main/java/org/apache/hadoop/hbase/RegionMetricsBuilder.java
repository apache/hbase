...
public final class RegionMetricsBuilder {
  ...
  public static List<RegionMetrics>
    toRegionMetrics(AdminProtos.GetRegionLoadResponse regionLoadResponse) {
  ...
  public static RegionMetrics toRegionMetrics(ClusterStatusProtos.RegionLoad regionLoadPB) {
    return RegionMetricsBuilder
      ...
      .setDataLocalityForSsd(
        regionLoadPB.hasDataLocalityForSsd() ? regionLoadPB.getDataLocalityForSsd() : 0.0f)
      .setBlocksLocalWeight(
        regionLoadPB.hasBlocksLocalWeight() ? regionLoadPB.getBlocksLocalWeight() : 0)
      .setBlocksLocalWithSsdWeight(
        regionLoadPB.hasBlocksLocalWithSsdWeight() ? regionLoadPB.getBlocksLocalWithSsdWeight() : 0)
      ...
      .setStoreFileIndexSize(new Size(regionLoadPB.getStorefileIndexSizeKB(), Size.Unit.KILOBYTE))
      .setStoreFileRootLevelIndexSize(
        new Size(regionLoadPB.getRootIndexSizeKB(), Size.Unit.KILOBYTE))
      .setStoreCount(regionLoadPB.getStores()).setStoreFileCount(regionLoadPB.getStorefiles())
      ...
      .setCurrentRegionPrefetchRatio(regionLoadPB.getCurrentRegionPrefetchRatio()).build();
  }

  private static List<ClusterStatusProtos.StoreSequenceId>
    toStoreSequenceId(Map<byte[], Long> ids) {
    return ids.entrySet().stream()
      .map(e -> ClusterStatusProtos.StoreSequenceId.newBuilder()
        .setFamilyName(UnsafeByteOperations.unsafeWrap(e.getKey())).setSequenceId(e.getValue())
        .build())
  ...
  public static ClusterStatusProtos.RegionLoad toRegionLoad(RegionMetrics regionMetrics) {
    return ClusterStatusProtos.RegionLoad.newBuilder()
      .setRegionSpecifier(HBaseProtos.RegionSpecifier.newBuilder()
        ...
        .setValue(UnsafeByteOperations.unsafeWrap(regionMetrics.getRegionName())).build())
      .setTotalStaticBloomSizeKB((int) regionMetrics.getBloomFilterSize().get(Size.Unit.KILOBYTE))
      ...
      .setStorefileIndexSizeKB((long) regionMetrics.getStoreFileIndexSize().get(Size.Unit.KILOBYTE))
      .setRootIndexSizeKB(
        (int) regionMetrics.getStoreFileRootLevelIndexSize().get(Size.Unit.KILOBYTE))
      .setStores(regionMetrics.getStoreCount()).setStorefiles(regionMetrics.getStoreFileCount())
      ...
      .setCurrentRegionPrefetchRatio(regionMetrics.getCurrentRegionPrefetchRatio()).build();
  }
  ...
  private float currentRegionPrefetchRatio;
  ...
  public RegionMetricsBuilder setCurrentRegionPrefetchRatio(float value) {
    this.currentRegionPrefetchRatio = value;
    return this;
  }
  ...
  public RegionMetrics build() {
    return new RegionMetricsImpl(name, storeCount, storeFileCount, storeRefCount,
      ...
      blocksTotalWeight, compactionState, regionSizeMB, currentRegionPrefetchRatio);
  }
  ...
  private static class RegionMetricsImpl implements RegionMetrics {
    ...
    RegionMetricsImpl(byte[] name, int storeCount, int storeFileCount, int storeRefCount,
      ...
      CompactionState compactionState, Size regionSizeMB, float currentRegionPrefetchRatio) {
      ...
      this.currentRegionPrefetchRatio = currentRegionPrefetchRatio;
    }
    ...
    public String toString() {
      ...
      Strings.appendKeyValue(sb, "storeFileCount", this.getStoreFileCount());
      ...
      Strings.appendKeyValue(sb, "uncompressedStoreFileSize", this.getUncompressedStoreFileSize());
      Strings.appendKeyValue(sb, "lastMajorCompactionTimestamp",
        this.getLastMajorCompactionTimestamp());
      Strings.appendKeyValue(sb, "storeFileSize", this.getStoreFileSize());
      if (this.getUncompressedStoreFileSize().get() != 0) {
        Strings.appendKeyValue(sb, "compressionRatio",
          String.format("%.4f", (float) this.getStoreFileSize().get(Size.Unit.MEGABYTE)
            / (float) this.getUncompressedStoreFileSize().get(Size.Unit.MEGABYTE)));
      }
      Strings.appendKeyValue(sb, "memStoreSize", this.getMemStoreSize());
      Strings.appendKeyValue(sb, "readRequestCount", this.getReadRequestCount());
      Strings.appendKeyValue(sb, "cpRequestCount", this.getCpRequestCount());
      Strings.appendKeyValue(sb, "writeRequestCount", this.getWriteRequestCount());
      Strings.appendKeyValue(sb, "rootLevelIndexSize", this.getStoreFileRootLevelIndexSize());
      Strings.appendKeyValue(sb, "uncompressedDataIndexSize",
        this.getStoreFileUncompressedDataIndexSize());
      Strings.appendKeyValue(sb, "bloomFilterSize", this.getBloomFilterSize());
      Strings.appendKeyValue(sb, "compactingCellCount", this.getCompactingCellCount());
      Strings.appendKeyValue(sb, "compactedCellCount", this.getCompactedCellCount());
      float compactionProgressPct = Float.NaN;
      if (this.getCompactingCellCount() > 0) {
        compactionProgressPct =
          ((float) this.getCompactedCellCount() / (float) this.getCompactingCellCount());
      }
      Strings.appendKeyValue(sb, "compactionProgressPct", compactionProgressPct);
      Strings.appendKeyValue(sb, "completedSequenceId", this.getCompletedSequenceId());
      Strings.appendKeyValue(sb, "dataLocality", this.getDataLocality());
      Strings.appendKeyValue(sb, "dataLocalityForSsd", this.getDataLocalityForSsd());
      Strings.appendKeyValue(sb, "blocksLocalWeight", blocksLocalWeight);
      ...
      Strings.appendKeyValue(sb, "currentRegionPrefetchRatio", currentRegionPrefetchRatio);
      return sb.toString();
    }
  }
