...
public class FixedFileTrailer {
   ...
   * @param istream the input stream with the ability to seek. Does not have to be buffered, as only
   *          one read operation is made.
  ...
  private static Class<? extends CellComparator> getComparatorClass(String comparatorClassName)
    ...
    if (comparatorClassName.equals(KeyValue.COMPARATOR.getLegacyKeyComparatorName())
      ...
      if (comparatorClassName.equals("org.apache.hadoop.hbase.KeyValue$RawBytesComparator")
          || comparatorClassName.equals("org.apache.hadoop.hbase.util.Bytes$ByteArrayComparator")) {
            // When the comparator to be used is Bytes.BYTES_RAWCOMPARATOR, we just return null from
            // here
            ...
            try {
              comparatorKlass =
                  (Class<? extends CellComparator>) Class.forName(comparatorClassName);
