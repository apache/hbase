...
public final class KeyStoreTestUtil {
  ...
  /**
   * Creates a keystore with a single key and saves it to a file. This method will use the same
   * password for the keystore and for the key. This method will always generate a keystore file in
   * JKS format.
  ...
  /**
   * Creates a keystore with a single key and saves it to a file. This method will always generate a
   * keystore file in JKS format.
   * @param filename String file to save
   * @param password String store password to set on keystore
  ...
  public static void createKeyStore(String filename, String password, String keyPassword,
      String alias, Key privateKey, Certificate cert) throws GeneralSecurityException, IOException {
   ...
   */
  public static void createKeyStore(String filename, String password, String keyPassword,
      String alias, Key privateKey, Certificate cert, String keystoreType)
      throws GeneralSecurityException, IOException {
   ...
   */
  public static void createTrustStore(String filename, String password, String alias,
      Certificate cert, String trustStoreType) throws GeneralSecurityException, IOException {
    KeyStore ks = createEmptyKeyStore(trustStoreType);
    ks.setCertificateEntry(alias, cert);
    saveKeyStore(ks, filename, password);
  }

  public static <T extends Certificate> void createTrustStore(String filename, String password,
  ...
  public static void cleanupSSLConfig(Configuration conf) throws Exception {
    ...
    String clientKeyStore =
        conf.get(FileBasedKeyStoresFactory.resolvePropertyName(SSLFactory.Mode.CLIENT,
          FileBasedKeyStoresFactory.SSL_KEYSTORE_LOCATION_TPL_KEY));
    if (clientKeyStore != null) {
      f = new File(clientKeyStore);
      f.delete();
    }
    f = new File(KeyStoreTestUtil.getClasspathDir(KeyStoreTestUtil.class) + "/"
        + conf.get(SSLFactory.SSL_CLIENT_CONF_KEY));
    f.delete();
    f = new File(KeyStoreTestUtil.getClasspathDir(KeyStoreTestUtil.class) + "/"
        + conf.get(SSLFactory.SSL_SERVER_CONF_KEY));
    f.delete();
  }

  /**
   * Performs complete setup of SSL configuration in preparation for testing an SSLFactory. This
   * includes keys, certs, keystores, truststores, the server SSL configuration file, the client SSL
   * configuration file, and the master configuration file read by the SSLFactory.
   * @param keystoresDir String directory to save keystores
   * @param sslConfDir String directory to save SSL configuration files
   * @param conf Configuration master configuration to be used by an SSLFactory, which will be
  ...
  public static void setupSSLConfig(String keystoresDir, String sslConfDir, Configuration conf,
    ...
    File sslClientConfFile = new File(sslConfDir + "/ssl-client-" + System.nanoTime() + "-"
        + HBaseCommonTestingUtility.getRandomUUID() + ".xml");
    File sslServerConfFile = new File(sslConfDir + "/ssl-server-" + System.nanoTime() + "-"
        + HBaseCommonTestingUtility.getRandomUUID() + ".xml");
