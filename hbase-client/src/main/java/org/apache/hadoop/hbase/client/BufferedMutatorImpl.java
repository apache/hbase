...
public class BufferedMutatorImpl implements BufferedMutator {
  ...
  BufferedMutatorImpl(ClusterConnection conn, BufferedMutatorParams params, AsyncProcess ap) {
    ...
    this.writeBufferSize = params.getWriteBufferSize() != UNSET ? params.getWriteBufferSize()
    ...
    this.maxKeyValueSize = params.getMaxKeyValueSize() != UNSET ? params.getMaxKeyValueSize()
        : tableConf.getMaxKeyValueSize();

    this.rpcTimeout = new AtomicInteger(params.getRpcTimeout() != UNSET ? params.getRpcTimeout()
        : conn.getConnectionConfiguration().getWriteRpcTimeout());

    this.operationTimeout =
        new AtomicInteger(params.getOperationTimeout() != UNSET ? params.getOperationTimeout()
  ...
  private AsyncProcessTask createTask(QueueRowAccess access) {
    return new AsyncProcessTask(
        AsyncProcessTask.newBuilder().setPool(pool).setTableName(tableName).setRowAccess(access)
            .setSubmittedRows(AsyncProcessTask.SubmittedRows.AT_LEAST_ONE).build()) {
      @Override
