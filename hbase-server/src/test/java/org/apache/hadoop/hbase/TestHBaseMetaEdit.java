...
public class TestHBaseMetaEdit {
  ...
  private TableDescriptor getMetaDescriptor() throws TableNotFoundException, IOException {
    ...
    TableDescriptor get = admin.getDescriptor(UTIL.getConnection().getMetaTableName());
  ...
  public void testEditMeta() throws IOException {
    Admin admin = UTIL.getAdmin();
    admin.tableExists(UTIL.getConnection().getMetaTableName());
    ...
    cfd = ColumnFamilyDescriptorBuilder.newBuilder(cfd).setMaxVersions(oldVersions + 1)
      ...
      .build();
    admin.modifyColumnFamily(UTIL.getConnection().getMetaTableName(), cfd);
    ...
    admin.addColumnFamily(UTIL.getConnection().getMetaTableName(), newCfd);
    ...
    admin.deleteColumnFamily(UTIL.getConnection().getMetaTableName(), newCfd.getName());
    descriptor = getMetaDescriptor();
    assertTrue(descriptor.getColumnFamily(newCfd.getName()) == null);
    try {
      admin.deleteColumnFamily(UTIL.getConnection().getMetaTableName(), HConstants.CATALOG_FAMILY);
  ...
  public void testAlterMetaWithReadOnly() throws IOException {
    ...
    try {
      ...
      assertFalse(admin.getDescriptor(UTIL.getConnection().getMetaTableName()).isReadOnly());
    }
