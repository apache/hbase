...
- and, for every test class selected for execution, it verifies that the class has exactly
  one sizing tag.
...
presence of exactly one sizing `@Tag` in every test class selected for execution; it does
not inspect classes excluded by tag filters).
