...
public class TestClientOperationTimeout {
  ...
  public void testMultiOperationTimoutWithLocationError() throws IOException, InterruptedException {
    ...
    try (Connection specialConnection = ConnectionFactory.createConnection(conf);
      ...
      // the batch to exceed the operation timeout, second action should then be failed fast with
      // OperationTimeoutExceededException
      Get firstAction = new Get(Bytes.toBytes(0)).addColumn(FAMILY, QUALIFIER);
      Get secondAction = new Get(Bytes.toBytes(1)).addColumn(FAMILY, QUALIFIER);
      List<Get> gets = Arrays.asList(firstAction, secondAction);
      try {
        specialTable.batch(gets, new Object[2]);
        Assert.fail("Should not reach here");
      ...
      // Test batch call with no result tracking, identical test to above call that does result
      // tracking
      // but does not assert that exceptions were tracked for the two different actions in the
      // batch,
      // just asserts that the two exceptions were tracked for *some* action in the batch.
      try {
        specialTable.batch(gets, null);
        Assert.fail("Should not reach here");
      } catch (RetriesExhaustedWithDetailsException exception) {
        Throwable firstActionCause = exception.getCause(0);
        Assert.assertTrue(firstActionCause instanceof RetriesExhaustedException);
      ...
      if (e.getMessage().contains("time out before the actionsInProgress changed to zero")) {
        Assert.fail("Not all action counters were decremented: " + e);
      }
      throw e;
    }
  }
