...
/*
 * This servlet is based off of the JMXProxyServlet from Tomcat 7.0.14. It has been rewritten to be
 * read only and to output in a JSON format so it is not really that close to the original.
 */
/**
    ...
    try {
      ...
      try {
        ...
        if (getmethod != null) {
          ...
          if (beanWriter.write(this.mBeanServer, new ObjectName(splitStrings[0]), splitStrings[1],
            description) != 0) {
            beanWriter.flush();
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
          }
