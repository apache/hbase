...
_settings.xml_ is configured correctly, as described in [maven.settings.xml](#maven.settings.xml).
...
[NOTE]
====
If you ran the old \_dev-support/make_rc.sh* script, this is as far as it takes you.
...
Make sure your _settings.xml_ is set up properly (see [maven.settings.xml](#maven.settings.xml)).
...
### Publishing [hbase.apache.org](https://hbase.apache.org)

See [website_publish](#website_publish) for instructions on publishing the website and documentation.
...
The tests were split into separate classes to accomodate class level timeouts (see [hbase.unittests](#hbase.unittests) for specifics).
...
[hbase.unittests](#hbase.unittests)). Medium tests are NOT executed if there is an error in a
...
This runs everything that is an integration test that matches _ClassX_. This means anything matching: "\**/IntegrationTest*ClassX\*". You can also run multiple groups of integration tests using comma-delimited lists (similar to unit tests). Using a list of matches still supports full regex matching for each of the groups. This would look something like:
...
ChaosMonkey defines _Actions_ and _Policies_.
...
Fix version of X.Y.0 => fixed in all releases X.Y'._ (where Y' = Y). +
Fix version of X.0.0 => fixed in all releases X'.\*._ (where X' = X). +
