...
public class TestWALSplitWithDeletedTableData {
  ...
  public void testWALSplitWithDeletedTableData() throws Exception {
    ...
    Thread.sleep(1 * 1000);
    while (cluster.hbaseCluster.getActiveMaster().getAssignmentManager().getRegionsInTransition()
        .size() > 0) {
      Thread.sleep(1 * 1000);
    }
    // Table 't1' is dropped. Assert table directory does not exist in WAL FS.
    tableDir = CommonFSUtils.getWALTableDir(TEST_UTIL.getConfiguration(), t1);
    assertFalse(TEST_UTIL.getDFSCluster().getFileSystem().exists(tableDir));
