...
Before you get started submitting code to HBase, please refer to [Developer Guidelines](#developing).

As Apache HBase is an Apache Software Foundation project, see [The Apache Software Foundation](#asf) for more information about how the ASF functions.
...
[Code Formatting Conventions](#common.patch.feedback).
...
- [AsciiDoc](https://plugins.jetbrains.com/plugin/7391-asciidoc/). HBase uses
...
With Eclipse set up as explained above in [Eclipse](#eclipse), you can also use the **Build** command in Eclipse.
...
See the [Running tests](#hbase.unittests.cmds) section in [Unit Tests](#hbase.unittests)
...
[Building against various Hadoop versions](#maven.build.hadoop). See [Java](#java) for a complete
support matrix of Java version by HBase version.
...
Continuing the discussion from the [earlier section](#maven.build.jdk11_hadoop3), building and
...
You can build a tarball without going through the release process described in [Releasing Apache HBase](#releasing), by running the following command:
...
For developing unit tests for your HBase applications, see [Unit Testing HBase Applications](#unit.tests).
...
If you are writing code for `hbase-server`, see [Unit Tests](#hbase.unittests) for how to write your tests.
...
The tests were split into separate classes to accomodate class level timeouts (see [Unit Tests](#hbase.unittests) for specifics).
...
Also, keep in mind that if you are running tests in the `hbase-server` module you will need to apply the maven profiles discussed in [Running tests](#hbase.unittests.cmds) to get the tests to run properly.
...
[Unit Tests](#hbase.unittests)). Medium tests are NOT executed if there is an error in a
...
- See [Unit Tests](#hbase.unittests) for test case categories and corresponding timeouts.
...
Make sure you review [Code Formatting Conventions](#common.patch.feedback) for code style. If your patch
...
See [Tests](#hbase.tests) for more information on tests.
...
```js
location.href='https://issues.apache.org/jira/browse/'+document.getElementsByClassName("js-issue-title")[0].innerHTML.match(/HBASE-\d+/)[0];
```
...
- [Update the documentation](#appendix_contributing_to_documentation) for new
...
- Maintain the shared Jenkins testing environment and other testing
  infrastructure.
- [Vote on release candidates](#hbase.rc.voting) after performing validation, even if non-binding.
...
See [Set Up Git](https://help.github.com/articles/set-up-git) if you need
pointers.
</Callout>
...
<Callout type="warning">
If you do not have all remote changes, the push will fail. If the push fails for any reason, fix
the problem or ask for help. Do not do a `git push --force`.
</Callout>
