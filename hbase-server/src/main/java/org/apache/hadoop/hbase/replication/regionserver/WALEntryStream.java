...
class WALEntryStream implements Closeable {
  ...
  private boolean readNextEntryAndRecordReaderPosition() throws IOException {
    ...
    if (logQueue.getQueueSize(walGroupId) > 1) {
      ...
      // the queue is already ended and quit.I the future, if we want to optimize the logic here,
      // for example, do not call this method every time, or do not acquire rollWriteLock in the
      // implementation of this method, we need to carefully review the optimized implementation.
