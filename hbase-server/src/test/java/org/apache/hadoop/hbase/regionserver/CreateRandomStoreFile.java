...
public class CreateRandomStoreFile {
  ...
  public boolean run(String[] args) throws IOException {
    ...
    configureKeyValue(numKV, Integer.parseInt(cmdLine.getOptionValue(KEY_SIZE_OPTION)),
    ...
    HFileContext meta =
      new HFileContextBuilder().withCompression(compr).withBlockSize(blockSize).build();
    StoreFileWriter sfw =
      new StoreFileWriter.Builder(conf, new CacheConfig(conf), fs).withOutputDir(outputDir)
        .withBloomType(bloomType).withMaxKeyCount(numKV).withFileContext(meta).build();
  ...
  public KeyValue generateKeyValue(long i) {
    ...
    return new KeyValue(k, 0, rowLen, k, rowLen, cfLen, k, rowLen + cfLen,
