...
public class TestWALSplit {
  ...
  public void testSplitPreservesEdits() throws IOException{
    ...
    Path originalLog = (getGeneratedWALs(OLDLOGDIR))[0].getPath();
  ...
  public void testSplitLeavesCompactionEventsEdits() throws IOException{
    ...
    useDifferentDFSClient();
    WALSplitter.split(HBASEDIR, WALDIR, OLDLOGDIR, fs, conf, wals);
    FileStatus[] oldWals = getGeneratedWALs(OLDLOGDIR);
    assertEquals(1, oldWals.length);
    Path originalLog = oldWals[0].getPath();
  ...
  public void testCorruptedLogFilesSkipErrorsFalseDoesNotTouchLogs()
    ...
    assertEquals("if skip.errors is false all files should remain in place",
        NUM_WRITERS, getGeneratedWALs(WALDIR).length);
  }
  ...
  public void testLogsGetArchivedAfterSplit() throws IOException {
    ...
    useDifferentDFSClient();
    WALSplitter.split(HBASEDIR, WALDIR, OLDLOGDIR, fs, conf, wals);
    FileStatus[] archivedLogs = getGeneratedWALs(OLDLOGDIR);
  ...
  private void retryOverHdfsProblem(final FileSystem spiedFs) throws Exception {
    ...
    try {
      ...
      assertEquals(NUM_WRITERS, getGeneratedWALs(OLDLOGDIR).length);
  ...
  public void testSplitLogFileEmpty() throws IOException {
    ...
    assertEquals(0, countWAL(getGeneratedWALs(OLDLOGDIR)[0].getPath()));
  }
  ...
  private FileStatus[] getGeneratedWALs(Path dir) throws IOException {
    return fs.listStatus(dir, new PathFilter() {
      @Override
      public boolean accept(Path path) {
        return path.getName().startsWith(WAL_FILE_PREFIX);
      }
    });
  }
  ...
  private int countWAL(Path log) throws IOException {
    LOG.info("Counting entries in " + log);
    int count = 0;
