...
The API changed in HBase 1.0. For connection configuration information, see [Client configuration and dependencies connecting to an HBase cluster](/docs/configuration#client-configuration-and-dependencies-connecting-to-an-hbase-cluster).
...
Information on non-Java clients and custom protocols is covered in [Apache HBase External APIs](/docs/external_apis)
