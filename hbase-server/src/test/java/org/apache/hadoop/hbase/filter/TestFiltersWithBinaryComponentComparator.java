...
public class TestFiltersWithBinaryComponentComparator {
  ...
  public void testRowFilterWithBinaryComponentComparator() throws IOException {
    ...
    for (Cell cell : result) {
      ...
      assertTrue(a == 1 && b > 10 && b < 20 && c > 90 && c < 100 && d == 1);
    }
    ht.close();
  }
  ...
  /**
   * Minicluster test to ensure that server wraps OffsetOutOfBoundsException in a
   * DoNotRetryIOException to prevent client retries
   */
  @Test
  public void testOutOfBoundsExceptionIsNotRetryable() throws IOException {
    ...
    byte[] shortIncompatibleValue = new byte[offsetValue];
    Arrays.fill(shortIncompatibleValue, (byte) 'A');
    Put put2 = new Put(Bytes.toBytes("row1"));
    put2.addColumn(family, qf, shortIncompatibleValue);
    table.put(put2);
  ...
  /**
   * Since we are trying to emulate SQL: SELECT * from table where a = 1 and b > 10 and b < 20 and c
   * > 90 and c < 100 and d = 1 We are generating rows with: a = 1, b >=9 and b < 22, c >= 89 and c
   * < 102, and d = 1 At the end the table will look something like this: ------------ a| b| c|d|
   * ------------ 1| 9| 89|1|family:qf|xyz| ----------- 1| 9| 90|1|family:qf|abc| ----------- 1| 9|
   * 91|1|family:qf|xyz| ------------------------- . ------------------------- .
  ...
  private void setRowFilters(FilterList filterList) {
    ...
    Filter b10Filter =
      new RowFilter(CompareOperator.GREATER, new BinaryComponentComparator(b10, bOffset));
    filterList.addFilter(b10Filter);

    byte[] b20 = Bytes.toBytes(20); // tests b < 20
    Filter b20Filter =
      new RowFilter(CompareOperator.LESS, new BinaryComponentComparator(b20, bOffset));
    ...
    Filter c90Filter =
      new RowFilter(CompareOperator.GREATER, new BinaryComponentComparator(c90, cOffset));
    filterList.addFilter(c90Filter);

    byte[] c100 = Bytes.toBytes(100); // tests c < 100
    Filter c100Filter =
      new RowFilter(CompareOperator.LESS, new BinaryComponentComparator(c100, cOffset));
    ...
    Filter dFilter =
      new RowFilter(CompareOperator.EQUAL, new BinaryComponentComparator(d1, dOffset));
  ...
  /**
   * We have rows with either "abc" or "xyz". We want values which have 'y' at second position of
   * the string. As a result only values with "xyz" shall be returned
   */
  private void setValueFilters(FilterList filterList) {
    int offset = 1;
    byte[] y = Bytes.toBytes("y");
    Filter yFilter =
      new ValueFilter(CompareOperator.EQUAL, new BinaryComponentComparator(y, offset));
