...
public class VisibilityScanDeleteTracker extends ScanDeleteTracker {
  ...
  /**
   * The inherited {@link ScanDeleteTracker#isRedundantDelete(ExtendedCell)} is label-blind: it
   * tracks deleteCell/deleteType/deleteTimestamp (and familyStamp) without regard to the visibility
   * tags of the delete markers. Reusing it during minor compaction could drop a delete marker that
   * only shadows differently-labeled data, resurrecting cells that should stay deleted. So on
   * cell-authorization tables we conservatively report no delete as redundant (matching the
